...
!
    ...
    ! added by Jing Chen, Sep 9 2012
     ...
     real(r8), pointer :: swnet_u_EASS(:)       ! urban net solar (shortwave) radiation(total) (W/m**2)
    !-----------------------------
!
!
! !OTHER LOCAL VARIABLES
...
!-----------------------------------------------------------------------
    ...
    swnet_u_EASS       => clm3%g%l%c%p%pef%swnet_u_EASS
...
!------------------------------------
    ...
    do fl = 1,num_urbanl
       ...
       do p = pfti(l), pftf(l)
          ...
          swnet_over_EASS(p)  = 0._r8
          swnet_under_EASS(p) = 0._r8
          swnet_grnd_EASS(p)  = sabg(p)
          swnet_EASS(p)       = swnet_over_EASS(p) + swnet_under_EASS(p) + swnet_grnd_EASS(p)
          swnet_u_EASS(p)     = swnet_EASS(p)

          clm3%g%l%c%p%pef%par_over_sunlit_EASS(p)  = 0._r8
          clm3%g%l%c%p%pef%par_over_shaded_EASS(p)  = 0._r8
          clm3%g%l%c%p%pef%par_under_sunlit_EASS(p) = 0._r8
          clm3%g%l%c%p%pef%par_under_shaded_EASS(p) = 0._r8
          !-------------------------          
...
!
    ...
    !-------------------------------
    ! added by Jing Chen ,Sep 10 2012
...
!
    ...
    real(r8), pointer :: qflx_evap_soi_EASS(:)     ! water(liquid) evaporation from grnd[mmH2O s-1]
...
! Determine fluxes from canyon surfaces
    ...
    do f = 1, num_urbanp
       ...
       qflx_trans_over_EASS(p)     = qflx_tran_veg(p)
       qflx_trans_under_EASS(p)    = 0._r8
       qflx_evap_over_EASS(p)      = qflx_evap_veg(p)
    ...
    do f = 1, num_urbanp
       ...
       !-------------
       ! added by Jing Chen, Sep 10 2012
       clm3%g%l%c%p%pes%Tleaf_new_over_sunlit_EASS(p)  = forc_t(g)
       clm3%g%l%c%p%pes%Tleaf_new_over_shaded_EASS(p)  = forc_t(g)
       clm3%g%l%c%p%pes%Tleaf_new_under_sunlit_EASS(p) = forc_t(g)
       clm3%g%l%c%p%pes%Tleaf_new_under_shaded_EASS(p) = forc_t(g)
       clm3%g%l%c%p%pes%Tleaf_over_EASS(p)  = forc_t(g)
       clm3%g%l%c%p%pes%Tleaf_under_EASS(p) = forc_t(g)
       !--------------------------------
       ...
       clm3%g%l%c%p%pps%ci_over_sunlit_EASS(p)  = 0._r8
       clm3%g%l%c%p%pps%ci_over_shaded_EASS(p)  = 0._r8
       clm3%g%l%c%p%pps%ci_under_sunlit_EASS(p) = 0._r8
       clm3%g%l%c%p%pps%ci_under_shaded_EASS(p) = 0._r8

       !co2 concentration on the surfaces of leaves
       clm3%g%l%c%p%pps%cs_over_sunlit_EASS(p)  = forc_pco2(g)
       clm3%g%l%c%p%pps%cs_over_shaded_EASS(p)  = forc_pco2(g)
       clm3%g%l%c%p%pps%cs_under_sunlit_EASS(p) = forc_pco2(g)
       clm3%g%l%c%p%pps%cs_under_shaded_EASS(p) = forc_pco2(g)
