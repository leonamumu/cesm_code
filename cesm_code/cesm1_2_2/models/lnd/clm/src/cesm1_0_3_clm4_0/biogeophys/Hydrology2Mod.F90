...
! !USES:
    ...
    use SnowHydrology_EASSMod, only : SnowCompaction_EASS, CombineSnowLayers_EASS,&
                        DivideSnowLayers_EASS, SnowWater_EASS, BuildSnowFilter_EASS
    use SoilHydrology_EASSMod, only : Infiltration_EASS, SoilWater_EASS, Drainage_EASS,&
                        SurfaceRunoff_EASS
    !----------------------
!
! !ARGUMENTS:
...
!
    ...
    !------------------
    ! added by Jing Chen Oct 22 2012
    real(r8), pointer :: dz_EASS(:,:)     ! layer thickness depth (m)
    real(r8), pointer :: zi_EASS(:,:)     ! interface depth (m)
...
!
    ...
    real(r8), pointer :: qflx_glcice_EASS(:)    ! flux of new glacier ice (mm H2O /s)
...
!
    ...
    !---------------------------------
    ! added by Jing Chen Jun 12 2013
    real(r8) :: psi_EASS,vwc_EASS,fsattmp_EASS     ! temporary variables for soilpsi calculation
    !----------------------------------
    ...
    !---------------------------------
    ! added by Jing Chen Jun 12 2013
    real(r8) :: watdry_EASS                ! temporary
    real(r8) :: rwat_EASS(lbc:ubc)         ! soil water wgted by depth to maximum depth of 0.5 m
    real(r8) :: swat_EASS(lbc:ubc)         ! same as rwat but at saturation
    real(r8) :: rz_EASS(lbc:ubc)           ! thickness of soil layers contributing to rwat (m)
    real(r8) :: tsw_EASS                   ! volumetric soil water to 0.5 m
    real(r8) :: stsw_EASS  
    ...
    !--------------
    ! added by Jing Chen Oct 22 2012
    real(r8) :: dhkdw_EASS(lbc:ubc,1:nlevgrnd)  ! d(hk)/d(vol_liq)
    real(r8) :: dwat_EASS(lbc:ubc,1:nlevgrnd)   ! change in soil water
    real(r8) :: hk_EASS(lbc:ubc,1:nlevgrnd)     ! hydraulic conductivity (mm h2o/s)
    real(r8) :: vol_liq_EASS(lbc:ubc,1:nlevgrnd)! partial volume of liquid water in layer
    real(r8) :: icefrac_EASS(lbc:ubc,1:nlevgrnd)! ice fraction in layer
    integer  :: num_snowc_EASS                  ! number of column snow points
    integer  :: num_nosnowc_EASS                ! number of column non-snow points
    integer  :: filter_nosnowc_EASS(ubc-lbc+1)  ! column filter for non-snow points
    integer  :: filter_snowc_EASS(ubc-lbc+1)    ! column filter for snow points
!-----------------------------------------------------------------------
    ...
    t_soi_10cm_EASS        => clm3%g%l%c%ces%t_soi_10cm_EASS
    z_EASS                 => clm3%g%l%c%cps%z_EASS
    dz_EASS                => clm3%g%l%c%cps%dz_EASS
    zi_EASS                => clm3%g%l%c%cps%zi_EASS
    begwb_EASS             => clm3%g%l%c%cwbal%begwb_EASS
    endwb_EASS             => clm3%g%l%c%cwbal%endwb_EASS
    h2ocan_EASS            => clm3%g%l%c%cws%pws_a%h2ocan_EASS
    ...
    h2osoi_liqice_10cm_EASS => clm3%g%l%c%cws%h2osoi_liqice_10cm_EASS
    ...
    qflx_runoff_u_EASS     => clm3%g%l%c%cwf%qflx_runoff_u_EASS
    qflx_runoff_r_EASS     => clm3%g%l%c%cwf%qflx_runoff_r_EASS
    ...
    t_grnd_u_EASS          => clm3%g%l%c%ces%t_grnd_u_EASS
    t_grnd_r_EASS          => clm3%g%l%c%ces%t_grnd_r_EASS
    ...
    qflx_evap_tot_EASS     => clm3%g%l%c%cwf%pwf_a%qflx_evap_tot_EASS
    qflx_snwcp_ice_EASS    => clm3%g%l%c%cwf%pwf_a%qflx_snwcp_ice_EASS
    soilpsi_EASS           => clm3%g%l%c%cps%soilpsi_EASS
    wf_EASS                => clm3%g%l%c%cps%wf_EASS
    ...
    call BuildSnowFilter_EASS(lbc, ubc, num_nolakec, filter_nolakec, &
                     num_snowc_EASS, filter_snowc_EASS, num_nosnowc_EASS, filter_nosnowc_EASS)

    ! Determine the change of snow mass and the snow water onto soil

    call SnowWater_EASS(lbc, ubc, num_snowc_EASS, filter_snowc_EASS, &
                     num_nosnowc_EASS, filter_nosnowc_EASS)

    ! Determine soil hydrology

    call SurfaceRunoff_EASS(lbc, ubc, lbp, ubp, num_hydrologyc, filter_hydrologyc, &
                       num_urbanc, filter_urbanc, vol_liq_EASS, icefrac_EASS )

    call Infiltration_EASS(lbc, ubc,  num_hydrologyc, filter_hydrologyc, &
                      num_urbanc, filter_urbanc)

    call SoilWater_EASS(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
                   num_urbanc, filter_urbanc, vol_liq_EASS, dwat_EASS, hk_EASS, dhkdw_EASS)

    call Drainage_EASS(lbc, ubc, num_hydrologyc, filter_hydrologyc, &
                  num_urbanc, filter_urbanc, vol_liq_EASS, hk_EASS, icefrac_EASS)

    if (.not. is_perpetual()) then

       ! Natural compaction and metamorphosis.

       call SnowCompaction_EASS(lbc, ubc, num_snowc_EASS, filter_snowc_EASS)

       ! Combine thin snow elements

       call CombineSnowLayers_EASS(lbc, ubc, num_snowc_EASS, filter_snowc_EASS)
 
       ! Divide thick snow elements

       call DivideSnowLayers_EASS(lbc, ubc, num_snowc_EASS, filter_snowc_EASS)
    else

       do fc = 1, num_snowc_EASS
           c = filter_snowc_EASS(fc)
           h2osno_EASS(c) = 0._r8
       end do
       do j = -nlevsno+1,0
           do fc = 1, num_snowc_EASS
               ...
               if (j >= snl_EASS(c)+1) then
                  h2osno_EASS(c) = h2osno_EASS(c) + h2osoi_ice_EASS(c,j) + h2osoi_liq_EASS(c,j)
               end if
           end do
        end do
    end if
    ...
    do j = -nlevsno+1,0
       do fc = 1, num_snowc_EASS
           c = filter_snowc_EASS(fc)
           if (j <= snl_EASS(c) .and. snl_EASS(c) > -nlevsno) then
               h2osoi_ice_EASS(c,j) = 0._r8
               h2osoi_liq_EASS(c,j) = 0._r8
               t_soisno_EASS(c,j)   = 0._r8
               dz_EASS(c,j)         = 0._r8
               z_EASS(c,j)          = 0._r8
               zi_EASS(c,j-1)       = 0._r8
           end if
    ...
    do fc = 1, num_nolakec
        c = filter_nolakec(fc)
        snowice_EASS(c) = 0._r8
        snowliq_EASS(c) = 0._r8
    end do
    ...
    do j = -nlevsno+1, 0
      do fc = 1, num_snowc_EASS
          c = filter_snowc_EASS(fc)
          if (j >= snl_EASS(c)+1) then
             snowice_EASS(c) = snowice_EASS(c) + h2osoi_ice_EASS(c,j)
             snowliq_EASS(c) = snowliq_EASS(c) + h2osoi_liq_EASS(c,j)
          end if
       end do
    end do
    ...
    do fc = 1, num_nolakec
       ...
       l = clandunit(c)
       if (ityplun(l) /= isturb) then
          t_soi_10cm_EASS(c) = 0._r8
          h2osoi_liqice_10cm_EASS(c) = 0._r8
       end if
    end do
    do j = 1, nlevsoi
      do fc = 1, num_nolakec
          ...
          l = clandunit(c)
          if (ityplun(l) /= isturb) then
             if (zi_EASS(c,j) <= 0.1_r8) then
                 fracl                      = 1._r8
                 t_soi_10cm_EASS(c)         = t_soi_10cm_EASS(c) + t_soisno_EASS(c,j)*dz_EASS(c,j)*fracl
                 h2osoi_liqice_10cm_EASS(c) = h2osoi_liqice_10cm_EASS(c) + (h2osoi_liq_EASS(c,j) &
                                 + h2osoi_ice_EASS(c,j)) * fracl
             else
                 if (zi_EASS(c,j) > 0.1_r8 .and. zi_EASS(c,j-1) .lt. 0.1_r8) then
                    fracl                      = (0.1_r8 - zi_EASS(c,j-1))/dz_EASS(c,j)
                    t_soi_10cm_EASS(c)         = t_soi_10cm_EASS(c) + t_soisno_EASS(c,j)*dz_EASS(c,j)*fracl
                    h2osoi_liqice_10cm_EASS(c) = h2osoi_liqice_10cm_EASS(c) + (h2osoi_liq_EASS(c,j) &
                                  + h2osoi_ice_EASS(c,j))* fracl
                 end if
    ...
    do fc = 1, num_nolakec
        ...
        if (ityplun(l) /= isturb) then
            t_soi_10cm_EASS(c) = t_soi_10cm_EASS(c)/0.1_r8
        end if
          if (ityplun(l)==isturb) then
            t_grnd_u_EASS(c) = t_soisno_EASS(c,snl(c)+1)
          end if
        if (ityplun(l)==istsoil .or. ityplun(l)==istcrop) then
            t_grnd_r_EASS(c) = t_soisno_EASS(c,snl_EASS(c)+1)
        end if
          if (ctype(c) == icol_roof .or. ctype(c) == icol_sunwall &
             .or. ctype(c) == icol_shadewall .or. ctype(c) == icol_road_imperv) then
            endwb_EASS(c) = h2ocan_EASS(c) + h2osno_EASS(c)
          else
            endwb_EASS(c) = h2ocan_EASS(c) + h2osno_EASS(c) + wa_EASS(c)
          end if
    end do
    ...
    do j = 1, nlevgrnd
      do fc = 1, num_nolakec
          c = filter_nolakec(fc)
           endwb_EASS(c) = endwb_EASS(c) + h2osoi_ice_EASS(c,j) + h2osoi_liq_EASS(c,j)
           h2osoi_vol_EASS(c,j) = h2osoi_liq_EASS(c,j)/(dz_EASS(c,j)*denh2o) + h2osoi_ice_EASS(c,j)/(dz_EASS(c,j)*denice)
       end do
    end do
    ...
    do fc = 1,num_nolakec
        ...
        g = cgridcell(c)
          if (ityplun(l)==istwet .or. ityplun(l)==istice      &
             ...
             qflx_irrig(c) = 0._r8
             qflx_surf_EASS(c)  = 0._r8
             qflx_infl_EASS(c)  = 0._r8
             qflx_qrgwl_EASS(c) = forc_rain(g) + forc_snow(g) - qflx_evap_tot_EASS(c) - qflx_snwcp_ice_EASS(c) - &
                             (endwb_EASS(c)-begwb_EASS(c))/dtime
             ! For dynamic topography, add meltwater from glacier_mec ice to the runoff.
             ! (Negative qflx_glcice => positive contribution to runoff)
             ! Note: The meltwater contribution is computed in PhaseChanges (part of Biogeophysics2).
             !       This code will not work if Hydrology2 is called before Biogeophysics2, or if
             ...
             if (glc_dyntopo .and. ityplun(l)==istice_mec) then
                qflx_qrgwl_EASS(c) = qflx_qrgwl_EASS(c) - qflx_glcice_EASS(c)   ! meltwater from melted ice
             endif
             fcov(c)       = spval
             fsat(c)       = spval
             qcharge_EASS(c)    = spval
             qflx_rsub_sat_EASS(c) = spval
          else if (ityplun(l) == isturb .and. ctype(c) /= icol_road_perv) then
             fcov(c)       = spval
             fsat(c)       = spval
             qcharge_EASS(c)    = spval
          ...
          if (ityplun(l)==istice_mec) then
             ...
             qflx_glcice_EASS(c) = qflx_glcice_EASS(c) + qflx_snwcp_ice_EASS(c)

             ! For dynamic topography, set qflx_snwcp_ice = 0 so that this ice mass does not run off.
             ! For static topography, qflx_glc_ice is passed to the ice sheet model, but the
             !  CLM runoff terms are not changed.

             if (glc_dyntopo) qflx_snwcp_ice_EASS(c) = 0._r8

          endif   ! istice_mec

        qflx_runoff_EASS(c) = qflx_drain_EASS(c) + qflx_surf_EASS(c) + qflx_qrgwl_EASS(c)
        if (ityplun(l)==istsoil .and. clm3%g%l%c%wtgcell(c) > 0.0_r8 ) then
           qflx_runoff_EASS(c) = qflx_runoff_EASS(c) - qflx_irrig(c)
        end if
          if (ityplun(l)==isturb) then
            qflx_runoff_u_EASS(c) = qflx_runoff_EASS(c)
          else if (ityplun(l)==istsoil .or. ityplun(l)==istcrop) then
            qflx_runoff_r_EASS(c) = qflx_runoff_EASS(c)
          end if
    end do
       ...
       do j = 1, nlevgrnd
          do fc = 1, num_hydrologyc
             ...
             if (h2osoi_liq_EASS(c,j) > 0._r8) then
                vwc_EASS = h2osoi_liq_EASS(c,j)/(dz_EASS(c,j)*denh2o)

                ! the following limit set to catch very small values of
                ! fractional saturation that can crash the calculation of psi

                fsattmp_EASS = max(vwc_EASS/vwcsat(c,j), 0.001_r8)
                psi_EASS = psisat(c,j) * (fsattmp_EASS)**bsw2(c,j)
                soilpsi_EASS(c,j) = min(max(psi_EASS,-15.0_r8),0._r8)
             else
                soilpsi_EASS(c,j) = -15.0_r8
             end if
          end do
       end do
#endif
       ...
       do j = 1, nlevgrnd
          do fc = 1, num_hydrologyc
             c = filter_hydrologyc(fc)
             !if (z(c,j)+0.5_r8*dz(c,j) <= 0.5_r8) then
             if (z_EASS(c,j)+0.5_r8*dz_EASS(c,j) <= 0.05_r8) then
                watdry_EASS = watsat(c,j) * (316230._r8/sucsat(c,j)) ** (-1._r8/bsw(c,j))
                rwat_EASS(c) = rwat_EASS(c) + (h2osoi_vol_EASS(c,j)-watdry_EASS) * dz_EASS(c,j)
                swat_EASS(c) = swat_EASS(c) + (watsat(c,j)    -watdry_EASS) * dz_EASS(c,j)
                rz_EASS(c) = rz_EASS(c) + dz_EASS(c,j)
             end if
          end do
      end do
       ...
       do fc = 1, num_hydrologyc
          c = filter_hydrologyc(fc)
          if (rz_EASS(c) /= 0._r8) then
             tsw_EASS  = rwat_EASS(c)/rz_EASS(c)
             stsw_EASS = swat_EASS(c)/rz_EASS(c)
          else
             watdry_EASS = watsat(c,1) * (316230._r8/sucsat(c,1)) ** (-1._r8/bsw(c,1))
             tsw_EASS = h2osoi_vol_EASS(c,1) - watdry_EASS
             stsw_EASS = watsat(c,1) - watdry_EASS
          end if
          wf_EASS(c) = tsw_EASS/stsw_EASS
       end do
#endif
...
!#              rootr_col_EASS(c,j)  = rootr_col(c,j)
...
!#              h2osoi_liqice_10cm_EASS(c)=h2osoi_liqice_10cm(c)
!#              qflx_drain_EASS(c)   = qflx_drain(c)
!#              qflx_infl_EASS(c)    = qflx_infl(c)
!#              qflx_qrgwl_EASS(c)   = qflx_qrgwl(c)
!#              qflx_rsub_sat_EASS(c)= qflx_rsub_sat(c)
!#              qflx_runoff_EASS(c)  = qflx_runoff(c)
!#              qflx_runoff_r_EASS(c)= qflx_runoff_r(c)
!#              qflx_runoff_u_EASS(c)= qflx_runoff_u(c)
!#              qflx_surf_EASS(c)    = qflx_surf(c)
!#              qflx_top_soil_EASS(c)= qflx_top_soil(c)
!#              snowdp_EASS(c)       = snowdp(c)
!#              snowice_EASS(c)      = snowice(c)
!3              snowliq_EASS(c)      = snowliq(c)
!#              t_grnd_EASS(c)       = t_grnd(c)
!#              t_grnd_r_EASS(c)     = t_grnd_r(c)
!#              t_grnd_u_EASS(c)     = t_grnd_u(c)
!#              t_soi_10cm_EASS(c)   = t_soi_10cm(c)
!#              zwt_EASS(c)          = zwt(c)
!#              qcharge_EASS(c)      = qcharge(c)
!#              qflx_glcice_EASS(c)  = qflx_glcice(c)
