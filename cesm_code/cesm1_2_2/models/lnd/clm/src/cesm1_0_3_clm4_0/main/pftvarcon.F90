...
!-----------------------------------------------------------------------------------------------
  ...
  real(r8):: clumping_pft_EASS(0:mxpft)  ! parameter1[2] Clumping_index in EASS
  real(r8):: lai_over_max_EASS(0:mxpft)  ! parameter1[8] LAI_max_overstory in EASS
  real(r8):: lai_under_max_EASS(0:mxpft) ! parameter1[9] LAI_max_understory in EASS
  real(r8):: albedo_snow_EASS(0:mxpft)   ! parameter1[12] albedo of new snow based on pft
  real(r8):: albedo_nir_EASS(0:mxpft)    ! parameter1[23] albedo of canopy without snow covered , nir band
  real(r8):: albedo_vis_EASS(0:mxpft)    ! parameter1[22] albedo of canopy without snow covered , vis band
  real(r8):: vcmax25_EASS(0:mxpft)       ! parameter1[36] maximum capacity of rubisco at 25C-Vcmax[umol CO2 m-2 s-1]
  real(r8):: Nleaf_EASS(0:mxpft)         ! parameter1[46] leaf Nitrogen content (mean value + 1 SD g/m2) 
  real(r8):: slope_vcmaxN_EASS(0:mxpft)  ! parameter1[47] slope of Vcmax-N curve
  real(r8):: h2o_ball_barry_EASS(0:mxpft)! parameter1[34] intercept for H2O ball berry
  real(r8):: kball_EASS(0:mxpft)         ! parameter1[33] k Ball
  real(r8):: hunder_EASS(0:mxpft)        ! parameter1[30] hight of understory[m]
  real(r8):: htop_EASS(0:mxpft)          ! parameter1[29] hight of  canopy[m]
  real(r8):: mp_EASS(0:mxpft)            ! slope of conductance-to-photosynthesis relationship
!-----------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------
     !added by Jing Chen, 26 March 2012
    call ncd_io('clumping_pft_EASS', clumping_pft_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('lai_over_max_EASS', lai_over_max_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('lai_under_max_EASS', lai_under_max_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )

    call ncd_io('albedo_snow_EASS', albedo_snow_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('albedo_nir_EASS', albedo_nir_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('albedo_vis_EASS', albedo_vis_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('vcmax25_EASS', vcmax25_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('Nleaf_EASS', Nleaf_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('slope_vcmaxN_EASS', slope_vcmaxN_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('h2o_ball_barry_EASS', h2o_ball_barry_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('kball_EASS', kball_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('hunder_EASS', hunder_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('htop_EASS', htop_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
    call ncd_io('mp_EASS',mp_EASS, 'read', ncid, readvar=readv, posNOTonfile=.true.)
    if ( .not. readv ) call endrun( trim(subname)//' ERROR: error in reading in pft data' )
!-----------------------------------------------------------------------------------------------
