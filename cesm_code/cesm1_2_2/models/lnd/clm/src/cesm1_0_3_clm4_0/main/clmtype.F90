...
type, public :: water_balance_type
   ...
   real(r8), pointer :: endwb_EASS(:)    ! water mass end of the time ste
   real(r8), pointer :: begwb_EASS(:)    !water mass begining of the time step
   !------------------
...
!Added by CHE Mingliang. 2013-11-15
   real(r8), pointer :: pgsi_DLM(:)   !potential growing season index (pgsi), unit: none, range: [0,1]
!  NOTE: pgsi=idxtemp*idxdaylen*idxpnpr*idxvpd
   real(r8), pointer :: pgsiday_DLM(:)
   real(r8), pointer :: idxtemp_DLM(:)    !index of temperature
   real(r8), pointer :: idxdaylen_DLM(:)  !index of daylength
!  real(r8), pointer :: idxpnpr_DLM(:)   !index of positive net photosynthesis rate
   real(r8), pointer :: idxvpd_DLM(:)     !index of vapor pressure deficit of the atmosphere
!  added by CHE M.L. 2014-11-11
   real(r8), pointer :: pft_DLM(:)

!  added by CHE M.L. 2014-11-15
   real(r8), pointer :: pgsiscnt_DLM(:) !counter for start of pgsi
   real(r8), pointer :: pgsiecnt_DLM(:) !counter for end of pgsi
!  added by CHE Mingliang, 2013-11-22
   real(r8), pointer :: lfstatuscur_DLM(:)      !current leaf status
   real(r8), pointer :: lfstatuspre_DLM(:)      !previous leaf status
   real(r8), pointer :: lfstatus1cnt_DLM(:)     !counter for leaf status 1
   real(r8), pointer :: lfstatus2cnt_DLM(:)     !counter for leaf status 2
   real(r8), pointer :: lfstatus3cnt_DLM(:)     !counter for leaf status 3
   real(r8), pointer :: lfstatus4cnt_DLM(:)     !counter for leaf status 4
![CML]or[cml] NOTE----------------------------------------------
...
!-----------------------------------------------------------------------------------------------
    ...
    real(r8), pointer :: elai_EASS(:)                ! elai in EASS
    real(r8), pointer :: clumping_EASS(:)            ! clumping index in EASS
    real(r8), pointer :: elai_over_EASS(:)           ! elai of overstory in EASS
    real(r8), pointer :: elai_over_shaded_EASS(:)    ! elai of shaded leaf in overstory canopy, EASS
    real(r8), pointer :: elai_over_sunlit_EASS(:)    ! elai of sunlit leaf in overstory canopy, EASS
    real(r8), pointer :: elai_under_EASS(:)          ! elai of understory in EASS
    real(r8), pointer :: elai_under_shaded_EASS(:)   ! elai of shaded leaf in understory canopy, EASS
    real(r8), pointer :: elai_under_sunlit_EASS(:)   ! elai of sunlit leaf in understory canopy, EASS
    real(r8), pointer :: esai_over_EASS(:)           ! esai of overstory canopy, EASS
    real(r8), pointer :: esai_under_EASS(:)          ! esai of understory canopy, EASS
    ...
    real(r8), pointer :: dTleaf_over_sunlit_EASS(:)  ! sunlit leaf temperature change, overstory[K]
    real(r8), pointer :: dTleaf_over_shaded_EASS(:)  ! shaded leaf temperature change, overstory[K]
    real(r8), pointer :: dTleaf_under_sunlit_EASS(:) ! sunlit leaf temperature change, understory[K]
    real(r8), pointer :: dTleaf_under_shaded_EASS(:) ! shaded leaf temperature change, understory[K]
    real(r8), pointer :: jmx_over_shaded_EASS(:)     ! light-saturated rate of electron transport
                                                     ! in the photosynthetic carbon reduction cycle
                                                     ! in shaded leaf cells, overstory[umol m-2 s-1]
    real(r8), pointer :: jmx_over_sunlit_EASS(:)     ! light-saturated rate of electron transport
                                                     ! in the photosynthetic carbon reduction cycle
                                                     ! in sunlit leaf cells, overstory[umol m-2 s-1]
    real(r8), pointer :: jmx_under_shaded_EASS(:)    ! light-saturated rate of electron transport
                                                     ! in the photosynthetic carbon reduction cycle
                                                     ! in shaded leaf cells, understory[umol m-2 s-1]
    real(r8), pointer :: jmx_under_sunlit_EASS(:)    ! light-saturated rate of electron transport
                                                     ! in the photosynthetic carbon reduction cycle
                                                     ! in sunlit leaf cells, understory[umol m-2 s-1]
    real(r8), pointer :: vcmx_over_shaded_EASS(:)    ! maximum rate of carboxylation in shaded leaves, overstoroy [umol CO2 m-2 s-1]
    real(r8), pointer :: vcmx_over_sunlit_EASS(:)    ! maximum rate of carboxylation in sunlit leaves, overstoroy [umol CO2 m-2 s-1]
    real(r8), pointer :: vcmx_under_shaded_EASS(:)   ! maximum rate of carboxylation in shaded leaves, understoroy [umol CO2 m-2 s-1]
    real(r8), pointer :: vcmx_under_sunlit_EASS(:)   ! maximum rate of carboxylation in sunlit leaves, understoroy [umol CO2 m-2 s-1]
...
!--------------------------------------------------------------------
   ...
   real(r8), pointer :: albedo_nir_EASS(:)     ! albedo of canopy without snow covered , nir band
   real(r8), pointer :: albedo_vis_EASS(:)     ! albedo of canopy without snow covered , vis band
   ...
   real(r8), pointer :: h2o_ball_barry_EASS(:) ! intercept for H2O ball berry                       parameter1[34] in readparam.c
   real(r8), pointer :: kball_EASS(:)          ! k Ball                                             parameter1[33] in readparam.c
   real(r8), pointer :: hunder_EASS(:)         ! hight of understory[m]
   real(r8), pointer :: htop_EASS(:)           ! hight of canopy[m]
...
type, public :: pft_estate_type
   ...
   real(r8), pointer :: cpairg_EASS(:)                 ! specific heat capacity of surface ground, depanding on saturated water vapor specific humidity [J kg-1 k-1]

!CML or [cml] NOTE---------------------------------  
! modified by CHE Mingliang, 2013-11-22
   real(r8), pointer:: vpd(:)         ! vapor presser deficit (vpd), unit: pa
   real(r8), pointer:: vpd_EASS(:)
...
type, public :: pft_eflux_type
   ...
   real(r8), pointer :: eflx_sh_grnd_r_EASS(:)    ! rural sensible heat flux from ground (W/m**2) [+ to atm]
   real(r8), pointer :: eflx_sh_grnd_u_EASS(:)    ! urban sensible heat flux from ground (W/m**2) [+ to atm]
   real(r8), pointer :: eflx_lh_tot_EASS(:)       ! total sensible heat flux [W m-2]
   real(r8), pointer :: eflx_sh_tot_EASS(:)       ! total sensible heat flux [W m-2]
   real(r8), pointer :: eflx_sh_tot_u_EASS(:)     ! urban total sensible heat flux (W/m**2) [+ to atm]
   real(r8), pointer :: eflx_sh_tot_r_EASS(:)     ! rural total sensible heat flux (W/m**2) [+ to atm]
   real(r8), pointer :: eflx_lh_tot_u_EASS(:)     ! urban total latent heat flux (W/m**2)  [+ to atm]
   real(r8), pointer :: eflx_lh_tot_r_EASS(:)     ! rural total latent heat flux (W/m**2)  [+ to atm]
   real(r8), pointer :: eflx_lh_can_EASS(:)       ! total latent heat flux from canopy[W m-2]
   real(r8), pointer :: eflx_lh_over_EASS(:)      ! total latent heat flux from overstroy[W m-2]
   real(r8), pointer :: eflx_lh_under_EASS(:)     ! total latent heat flux from understroy[W m-2]
   real(r8), pointer :: eflx_lh_sunlit_EASS(:)    ! total latent heat flux from sunlit leaves[W m-2]
   real(r8), pointer :: eflx_lh_shaded_EASS(:)    ! total latent heat flux from shaded leaves[W m-2]
   real(r8), pointer :: eflx_lh_grnd_EASS(:)      ! total latent heat flux from ground surface[W m-2]
   real(r8), pointer :: eflx_sh_can_EASS(:)       ! total sensible heat flux from ground surface[W m-2]
   ...
   real(r8), pointer :: lwnet_u_EASS(:)           ! urban net infrared (longwave) rad (W/m**2) [+ = to atm]
   real(r8), pointer :: lwnet_r_EASS(:)           ! rural net infrared (longwave) rad (W/m**2) [+ = to atm]
   real(r8), pointer :: lwrad_grnd_EASS(:)        ! net longwave radiation of grnd[W m-2]
   real(r8), pointer :: swnet_EASS(:)             ! net solar (shortwave) radiation [W m-2]
   real(r8), pointer :: swnet_u_EASS(:)           ! rural net solar (shortwave) radiation [W m-2]
   ...
   real(r8), pointer :: eflx_lh_veg(:)            ! veg heat flux (W/m**2) [+ to atm](=evaporation+transpiration)
   ...
   real(r8), pointer :: eflx_soil_grnd_EASS(:)    ! soil heat flux (W/m**2) [+ = into soil]
   real(r8), pointer :: eflx_soil_grnd_r_EASS(:)  ! rural soil heat flux (W/m**2) [+ = into soil]
   real(r8), pointer :: eflx_soil_grnd_u_EASS(:)  ! urban soil heat flux (W/m**2) [+ = into soil]
   real(r8), pointer :: eflx_gnet_EASS(:)         !net heat flux into ground (W/m**2)
   real(r8), pointer :: dgnetdT_EASS(:)           !derivative of net ground heat flux wrt soil temp (W/m**2 K)
   !--------------------
...
type, public :: pft_wflux_type
   ...
   ! added by Jing Chen, 17 May 2012
   ! variables for EASS code
   real(r8), pointer :: snow_grnd_EASS(:)      !snow rate on the ground [m s-1]
   real(r8), pointer :: sum_rain_over_EASS(:)  ! sum of rainfall on the overstory [m s-1]
   real(r8), pointer :: sum_rain_under_EASS(:) ! sum of rainfall on the understory[m s-1]
   real(r8), pointer :: sum_snow_over_EASS(:)  ! sum of snow on the overstory[m s-1]
   real(r8), pointer :: sum_snow_under_EASS(:) ! sum of snow on the understory[m s-1]
   ...
   real(r8), pointer :: qflx_evap_soi_EASS(:)            ! evaporation from soil[mmH2O s-1]
   ...
   real(r8), pointer :: qflx_evap_tot_EASS(:)           ! total water vapor heat flux [mmH2O s-1]

   real(r8), pointer :: qflx_evap_can_EASS(:)      ! total water vapor heat flux from canopy[mmH2O s-1]
   real(r8), pointer :: qflx_evtr_sunlit_EASS(:)   ! total water vapor heat flux from sunlit leaves[mmH2O s-1]
   real(r8), pointer :: qflx_evtr_shaded_EASS(:)   ! total water vapor heat flux from shaded leaves[mmH2O s-1]
   real(r8), pointer :: qflx_evap_grnd_EASS(:)     ! total water vapor heat flux from ground surface[mmH2O s-1]
   ...
   real(r8), pointer :: qflx_snow_grnd_EASS(:)     !snow rate on the grond at column-level  [m s-1]
   real(r8), pointer :: qflx_tran_veg_EASS(:)      !vegetation transpiration (mm H2O/s) (+ = to atm)
   !---------------------------------
...
type, public :: pft_cflux_type
   ...
   real(r8), pointer :: gpp_fpsn_EASS(:)           ! GPP flux based on photosynthesis in EASS [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn_over_EASS(:)      ! GPP flux of overstorybased on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn_under_EASS(:)     ! GPP flux of understorybased on photosynthesis [gC m-2 s-1]
   ...
   real(r8), pointer :: gpp_fpsn(:)                ! GPP flux based on photosynthesis in CLM[gC m-2 s-1]
...
type, public :: column_pstate_type
   ...
   real(r8), pointer :: tksat0(:,:)           !parameter for caculating the dksat, added by Shaobo Sun, Ref. Chadburn et al. 2015
...
!  real(r8), pointer :: xirrig(:)             !irrigation rate
   ...
   ! variables for EASS code
   real(r8), pointer :: albsnow_EASS(:,:)      ! albedo of new snow (1=vis,2=nir)
   real(r8), pointer :: alb_can_EASS(:,:)      ! albedo of canopy without snow covered (1=vis,2=nir)
   real(r8), pointer :: albsno_over_EASS(:,:)  ! albedo of the overstroy covered by snow (1=vis,2=nir)
   real(r8), pointer :: albsno_under_EASS(:,:) ! albedo of the understory covered by snow (1=vis,2=nir)
   real(r8), pointer :: alb_grnd_EASS(:,:)     ! albedo of ground(1=vis,2=nir)
   real(r8), pointer :: albsno_grnd_EASS(:,:)  ! albedo of the ground covered by snow (1=vis,2=nir)
   ...
   real(r8), pointer :: snowdp_EASS(:)           ! snow height on the ground [m]
   ...
   integer , pointer :: imelt_EASS(:,:)          !flag for melting (=1), freezing (=2), Not=0 (new) (-nlevsno+1:nlevgrnd)
   real(r8), pointer :: eff_porosity_EASS(:,:)   !effective porosity = porosity - vol_ice (nlevgrnd)
   real(r8), pointer :: fracice_EASS(:,:)        !fractional impermeability (-)
   real(r8), pointer :: soilpsi_EASS(:,:)        !soil water potential in each soil layer (MPa)
   real(r8), pointer :: wf_EASS(:)               !soil water as frac. of whc for top 0.5 m
...
type, public :: column_estate_type
   ...
   real(r8), pointer :: t_grnd_r_EASS(:)     ! Rural ground temperature (Kelvin)			
   real(r8), pointer :: t_grnd_u_EASS(:)     ! Urban ground temperature (Kelvin)			
   real(r8), pointer :: t_soi_10cm_EASS(:)   ! soil temperature in top 10cm of soil (Kelvin)			
   real(r8), pointer :: t_soisno_EASS(:,:)   ! soil temperature (Kelvin)			
   real(r8), pointer :: tssbef_EASS(:,:)     ! soil/snow temperature before update
   real(r8), pointer :: hc_soi_EASS(:)       ! soil heat content (MJ/m2)
   real(r8), pointer :: hc_soisno_EASS(:)    ! soil plus snow heat content (MJ/m2)
   real(r8), pointer :: t_grnd_bef_EASS(:)   ! ground temperature before update(Kelvin)
   real(r8), pointer :: t_grnd_bef(:)        ! ground temperature before update(Kelvin)
   !-------------------
...
type, public :: column_wstate_type
   ...
   real(r8), pointer :: h2osoi_liqice_10cm_EASS(:) ! liquid water + ice lens in top 10cm of soil (kg/m2)			
...
type, public :: column_eflux_type
   ...
   ! added by Jing Chen Oct 22 2012
   ! variables for EASS code
   real(r8), pointer :: eflx_bot_EASS(:)      ! heat flux from beneath soil/ice column (W/m2)	
   real(r8), pointer :: eflx_snomelt_EASS(:)  ! snow melt heat flux (W/m2)
   real(r8), pointer :: eflx_snomelt_r_EASS(:)! rural snow melt heat flux (W/m**2)
   real(r8), pointer :: eflx_fgr12_EASS(:)    ! ground heat flux between soil layers 1 and 2 (W/m2)
...
type, public :: column_wflux_type
   ...
   ! variables for EASS code
    ...
    real(r8), pointer :: qflx_runoff_r_EASS(:)    ! Rural total runoff (qflx_drain+qflx_surf+qflx_qrgwl) (mm H2O /s)			
    real(r8), pointer :: qflx_runoff_u_EASS(:)    ! Urban total runoff (qflx_drain+qflx_surf) (mm H2O /s)			
    ...
    real(r8), pointer :: qflx_snofrz_lyr_EASS(:,:)! snow freezing rate (positive definite) (col,lyr) [kg m-2 s-1]
   !--------------------------------
