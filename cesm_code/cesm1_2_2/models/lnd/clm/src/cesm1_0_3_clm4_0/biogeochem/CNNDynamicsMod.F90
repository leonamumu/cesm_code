...
!
   real(r8), pointer :: ndep_to_sminn(:)  ! the atmospheric N deposition goes to the soil mineral N pool.
!
...
!
! June 12 2013 : Jing Chen, change interface to EASS(DLM) 
!
! !USES:
   use clmtype
...
! local pointers to implicit in scalars
!
   real(r8), pointer :: h2osoi_liq_EASS(:,:)  ! liquid water (kg/m2) (new) (-nlevsno+1:nlevgrnd)
...
!-----------------------------------------------------------------------
   ...
   do j = 1,nlevsoi
      do fc = 1,num_soilc
         c = filter_soilc(fc)
         ! change h2osoi_liq(c) to h2osoi_liq_EASS(c)  by Jing Chen June 13 2013
         tot_water(c) = tot_water(c) + h2osoi_liq_EASS(c,j)
      end do
   end do
   ...
   do fc = 1,num_soilc
      ...
      ! change qflx_drain(c) to qflx_drain_EASS(c)  by Jing Chen June 13 2013
      sminn_leached(c) = disn_conc * qflx_drain_EASS(c)
