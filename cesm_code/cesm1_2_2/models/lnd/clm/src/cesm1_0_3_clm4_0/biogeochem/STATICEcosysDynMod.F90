...
!-----------------------------------------------------------------------------------------------
  ...
  real(r8), private, allocatable :: mclumping_EASS2t(:,:) ! clumping index for interpolation (2 months)
!-----------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------
    ...
    logical :: understory_index= .false.       ! false=> close all understroy calulation, consider overstory only
                                               ! true=> open all understroy calulation
...
!------------------------------------------------------------------
    ...
    real(r8), pointer :: elai_over_EASS(:)             ! elai_EASS of overstory
    real(r8), pointer :: elai_under_EASS(:)            ! elai_EASS of understory
    real(r8), pointer :: esai_over_EASS(:)             ! esai_EASS of overstory
    real(r8), pointer :: esai_under_EASS(:)            ! esai_EASS of understory
!-------------------------------------------------------------------
!
...
!------------------------------------------------------------------------
     !added by Jing Chen, 26 March 2012
          clumping_EASS(p) = timwt(1)*mclumping_EASS2t(p,1) + timwt(2)*mclumping_EASS2t(p,2)
!------------------------------------------------------------------------
...
!-----------------------------------------------------------------------
     !added by Jing Chen, 26 March 2012
          ...
          if (clumping_EASS(p)<=0._r8) then
                clumping_EASS(p)=0.5_r8
          else if (clumping_EASS(p)>1.0_r8) then
                clumping_EASS(p)=1.0_r8
        ! else 
        !	clumping_EASS(p)= clumping_EASS(p)
          end if
        ...
        ! recalculate LAI 
          ...
          elai_EASS(p)=elai_EASS(p) *clumping_pft_EASS(ivt(p))/clumping_EASS(p) 


        ! divide the elai and esai into overstory(required) and understory(optinal)
        ...
        ! close understory (optional)

        ! if (understory_index = .false.) then
        !	elai_under_EASS(p)=0._r8
        !       esai_under_EASS(p)=0._r8
        ! end if
...
!-------------------------------------------------------------------
    !added by Jing Chen, 26 March 2012
    real(r8), pointer :: mclumping_EASS(:,:)        ! lai read from input files
    real(r8):: mclumping_const                       ! set clumping index to a constent
    logical :: mclumping_const_index=.false.        ! true=> not read var. from input data file, then considered as a constent
!----------------------------------------------------------------
...
!----------------------------------------------------------------
            !added by Jing Chen, 26 March 2012
             mclumping_EASS(begg:endg,0:numpft), &
!----------------------------------------------------------------
...
!-------------------------------------------------------------
     !added by Jing Chen, 26 March 2012
       ...
       mclumping_const_index=.false.

       call ncd_io(ncid=ncid, varname='MONTHLY_CLUMPING_EASS', flag='read', data=mclumping_EASS, dim1name=grlnd, &
            nt=months(k), readvar=readvar)
     ! if (.not. readvar) call endrun( trim(subname)//' ERROR: MONTHLY_CLUMPING_EASS NOT on fveg file' )
       if (.not. readvar) then
                mclumping_const_index=.true.
                mclumping_const=0.55_r8  
       end if
!--------------------------------------------------------------
...
!--------------------------------------------------------------
                  !added by Jing Chen, 26 March 2012
                   if (mclumping_const_index == .true.) mclumping_EASS(g,l) = mclumping_const      ! optinal, if (.not.readvar)
                   mclumping_EASS2t(p,k) = mclumping_EASS(g,l)
!--------------------------------------------------------------
...
!---------------------------------------------------------
    ...
    deallocate(mlai, msai, mclumping_EASS, mhgtt, mhgtb)
