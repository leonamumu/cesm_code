...
!-----------------------------------------------------------------------------------------------
                           ...
                           clumping_pft_EASS, lai_over_max_EASS, lai_under_max_EASS, &
                           albedo_nir_EASS, albedo_vis_EASS, albedo_snow_EASS,vcmax25_EASS,&
                           Nleaf_EASS, kball_EASS, slope_vcmaxN_EASS, h2o_ball_barry_EASS,&
                           hunder_EASS, htop_EASS, mp_EASS,  &
!-----------------------------------------------------------------------------------------------
...
!
  ...
  real(r8), pointer :: tksat0(:,:)        ! parameter for caculating dksat (nlevgrnd), added by Shaobo Sun
  ...
  !-------------------
  ! added by Jing Chen Oct 24 2012
...
! real(r8) :: om_watsat    = 0.766_r8    ! Add by Shaobo Sun, REF(van Genuchten, et al., 1980; Balsamo, et al., 2009) 
  real(r8) :: om_hksat     = 0.00093_r8  ! Add by Shaobo Sun, REF(van Genuchten, et al., 1980; Balsamo, et al., 2009) 
...
!------------------------------------------------------------------------
  ...
  ! --------------------------------------------------------------------
   ...
   do m = 0,numpft
     ...
     !------------------------------------------------
     !added by Jing Chen, 26 March 2012
     pftcon%clumping_pft_EASS(m) = clumping_pft_EASS(m) 
     pftcon%lai_over_max_EASS(m) = lai_over_max_EASS(m) 
     pftcon%lai_under_max_EASS(m) = lai_under_max_EASS(m) 

     pftcon%albedo_nir_EASS(m) = albedo_nir_EASS(m)
     pftcon%albedo_vis_EASS(m) = albedo_vis_EASS(m)
     pftcon%albedo_snow_EASS(m) = albedo_snow_EASS(m)

     pftcon%vcmax25_EASS(m) = vcmax25_EASS(m)
     pftcon%Nleaf_EASS(m) = Nleaf_EASS(m)
     pftcon%kball_EASS(m) = kball_EASS(m)
     pftcon%slope_vcmaxN_EASS(m) = slope_vcmaxN_EASS(m)
     pftcon%h2o_ball_barry_EASS(m) = h2o_ball_barry_EASS(m)
     pftcon%hunder_EASS(m) = hunder_EASS(m)
     pftcon%htop_EASS(m) = htop_EASS(m)
     pftcon%mp_EASS(m) = mp_EASS(m)
...
!            tkm           = (1._r8-om_frac)*(4.80_r8*sand+1.92_r8*clay)/(sand+clay)+om_tkm*om_frac ! W/(m K)   !revised by Shaobo Sun
           ...
           ! hksat(c,lev)  = uncon_frac*uncon_hksat + (perc_frac*om_frac)*om_hksat
            hksat(c,lev)  = (1._r8-om_frac)*xksat+om_frac*om_hksat                                               ! add by Shaobo Sun
...
!             tkmg(c,lev)   = tkm * (1._r8- watsat(c,lev))                                                        ! add by Shaobo Sun, 2016/08/14
!             tksatu(c,lev) = tkmg(c,lev) + 0.57_r8*watsat(c,lev)                                                  ! add by Shaobo Sun, 2016/08/14

!            tkdry(c,lev)  = ((0.135_r8*bd + 64.7_r8) / (2.7e3_r8 - 0.947_r8*bd))*(1._r8-om_frac) + &
!                            om_tkd*om_frac

!   Revised by Shaobo Sun, 2016/8/21
!   Ref. Chadburn et al. 2015             
            tkdry(c,lev) = (((0.135_r8*bd + 64.7_r8) / (2.7e3_r8 - 0.947_r8*bd))**(1._r8-om_frac))*(om_tkd**(om_frac))
              if (tkdry(c,lev) .lt. 0.06_r8) then
                 tksat0(c,lev) = 0.5_r8
              else if (tkdry(c,lev) > 0.06_r8 .and. tkdry(c,lev) < 0.3_r8) then
                 tksat0(c,lev)= (1._r8-0.0134_r8*dlog(tkdry(c,lev)))/(-0.745_r8-dlog(tkdry(c,lev)))
              else
                 tksat0(c,lev) = 2.2_r8
              end if           
      ...
      if (ltype(l) == istdlak) then
         ...
         !----------------
         ! added by Jing Chen Oct 24 2012
         z_EASS(c,1:nlevgrnd)  = z(c,1:nlevgrnd)
         zi_EASS(c,0:nlevgrnd) = zi(c,0:nlevgrnd)
         dz_EASS(c,1:nlevgrnd) = dz(c,1:nlevgrnd)
         !----------------
      end if
