...
!
   ...
   !--------------------------
   ! added by Jing Chen June 12 2013
...
!-----------------------------------------------------------------------
   ...
   do fp=1,num_soilp
      ...
      !--------------------------
      !# changed by Jing Chen, Oct 4 2012
!#      psnsun_to_cpool(p) = psnsun(p) * laisun(p) * 12.011e-6_r8
!#      psnshade_to_cpool(p) = psnsha(p) * laisha(p) * 12.011e-6_r8

       psnsun_to_cpool(p) = gpp_fpsn_sunlit_EASS(p)
       psnshade_to_cpool(p) = gpp_fpsn_shaded_EASS(p)
       !-------------------------------
   ...
   do fp=1,num_soilp
      ...
      if (gpp(p) > 0.0_r8) then
         ...
         !------------------
         !# removed by Jing Chen, Nov 6 2013
         !# psnsun_to_cpool(p) = psnsun_to_cpool(p)*(1._r8 - downreg(p))     !# removed by Chenjinn
         !# psnshade_to_cpool(p) = psnshade_to_cpool(p)*(1._r8 - downreg(p)) !# removed by Chenjinn
         !-----------------------
#if (defined C13)
