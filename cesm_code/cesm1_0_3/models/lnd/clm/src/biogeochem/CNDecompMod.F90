...
! !REVISION HISTORY:
! 8/15/03: Created by Peter Thornton
! June 12 2013, Jing Chen: convert interface to EASS(DLM)
!
! !LOCAL VARIABLES:
...
!
   ...
   real(r8), pointer :: dz_EASS(:,:)          ! soil layer thickness (m)
...
!-----------------------------------------------------------------------
   ...
   do j = 1,nlevdecomp
      do fc = 1,num_soilc
         c = filter_soilc(fc)
         t_scalar(c)=t_scalar(c) + (1.5**((t_soisno_EASS(c,j)-(SHR_CONST_TKFRZ+25._r8))/10._r8))*fr(c,j)
      end do
   end do
   ...
   do j = 1,nlevdecomp
      do fc = 1,num_soilc
         c = filter_soilc(fc)
         maxpsi = psisat(c,j)
         psi = min(soilpsi_EASS(c,j),maxpsi)
