...
!
   ...
   ! pft level
   real(r8), pointer :: t_ref2m(:)    ! 2 m height surface air temperature (Kelvin)   !???should be converted to EASS 
...
!-----------------------------------------------------------------------
   ...
   do j=1,nlevgrnd
      do fc = 1, num_soilc
         ...
         ! changed t_soisno(c,j) to t_soisno_EASS(c,j) by Jing Chen June 15 2013
         tcsoi(c,j) = q10**((t_soisno_EASS(c,j)-SHR_CONST_TKFRZ - 20.0_r8)/10.0_r8)
      end do
   end do
