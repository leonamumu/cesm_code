...
! !USES:
    ...
    use clm_varcon   , only : icol_roof, icol_sunwall, icol_shadewall, icol_road_perv, &
                              icol_road_imperv, istsoil, istcrop
!
! !ARGUMENTS:
    implicit none
...
!
    ...
    !-------------------
    ! added by Jing Chen Oct 29 2012
...
!-----------------------------------------------------------------------
    ...
    call p2c(num_nolakec, filter_nolakec, h2ocan_pft_EASS, h2ocan_col_EASS)
    ...
    do f = 1, num_hydrologyc
       ...
       if(zwt_EASS(c) <= zi_EASS(c,nlevsoi)) then
          wa_EASS(c) = 5000._r8
       end if
    end do
    ...
    do f = 1, num_nolakec
       ...
       if (ctype(c) == icol_roof .or. ctype(c) == icol_sunwall &
         ...
         begwb_EASS(c) = h2ocan_col_EASS(c) + h2osno_EASS(c)
       else
         begwb(c) = h2ocan_col(c) + h2osno(c) + wa(c)
         begwb_EASS(c) = h2ocan_col_EASS(c) + h2osno_EASS(c) + wa_EASS(c)
       end if
    end do
    do j = 1, nlevgrnd
      do f = 1, num_nolakec
         ...
         begwb_EASS(c) = begwb_EASS(c) + h2osoi_ice_EASS(c,j) + h2osoi_liq_EASS(c,j)
      end do
    end do
...
! !USES:
    ...
    use clm_varcon   , only : isturb,istdlak, istslak, icol_roof, icol_sunwall, icol_shadewall, &
                              spval, icol_road_perv, icol_road_imperv, istice_mec, istsoil, istcrop
...
!
    ...
    !-------------------------
    ! added by Jing Chen, Sep 10 2012
    real(r8), pointer :: radnet_EASS(:)     ! net radiation [W m-2]
    !---------------------------
    ...
    do p = lbp, ubp
       ...
       if (pwtgcell(p)>0._r8 .or. ltype(l)==istice_mec) then
          ...
          if ( frac_veg_nosno(p) /= 0 .and. ( ltype(l) == istsoil .or. ltype(l) == istcrop) ) then
                swnet_EASS(p) = swnet_EASS(p)
                lwnet_EASS(p) = lwnet_EASS(p)
          else
               swnet_EASS(p) = fsa(p)
               lwnet_EASS(p) = -eflx_lwrad_net(p)
          end if          

          radnet_EASS(p)=swnet_EASS(p)+lwnet_EASS(p)
          !----------------------------------
