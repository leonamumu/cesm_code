...
! !PUBLIC MEMBER FUNCTIONS:
  public :: SoilTemper_EASS       ! Snow and soil temperatures including phase change
!
! !PRIVATE MEMBER FUNCTIONS:
  private :: SoilThermProp_EASS   ! Set therm conductivities and heat cap of snow/soil layers
  private :: PhaseChange_EASS     ! Calculation of the phase change within snow and soil layers
!
! !REVISION HISTORY:
...
!BOP
!
! !IROUTINE: SoilTemper_EASS
!
! !INTERFACE:
  subroutine SoilTemper_EASS(lbl, ubl, lbc, ubc, num_urbanl, filter_urbanl, &
...
! !USES:
    ...
    use Tridiagonal_EASSMod, only : Tridiagonal_EASS
    !-------------
...
!
    ...
    real(r8), pointer :: lwrad_grnd_EASS(:) ! net solar (shortwave) radiation of grnd[W m-2]
    integer , pointer :: frac_veg_nosno(:)  ! fraction of vegetation not covered by snow (0 OR 1 now) [-] (new)
    real(r8), pointer :: eflx_sh_grnd(:)   ! sensible heat flux from ground (W/m**2) [+ to atm]
    real(r8), pointer :: qflx_evap_soi(:)   ! soil evaporation (mm H2O/s) (+ = to atm)
    real(r8), pointer :: qflx_evap_soi_EASS(:)  !  water vapor heat flux from ground surface[mmH2O s-1]
    real(r8), pointer :: eflx_sh_grnd_EASS(:)    ! sensible heat flux, ground [w m-2]
!#urban    real(r8), pointer :: qflx_tran_veg(:) ! vegetation transpiration (mm H2O/s) (+ = to atm) 
...
!#urban    real(r8), pointer :: eflx_lwrad_net(:)  ! net infrared (longwave) rad (W/m**2) [+ = to atm]
    real(r8), pointer :: tssbef_EASS(:,:)        ! temperature at previous time step [K]
!#urban    real(r8), pointer :: t_building(:)      ! internal building temperature (K)
!#urban    real(r8), pointer :: t_building_max(:)  ! maximum internal building temperature (K)
!#urban    real(r8), pointer :: t_building_min(:)  ! minimum internal building temperature (K)
    real(r8), pointer :: hc_soi_EASS(:)          ! soil heat content (MJ/m2)
    real(r8), pointer :: hc_soisno_EASS(:)       ! soil plus snow plus lake heat content (MJ/m2)
    real(r8), pointer :: eflx_fgr12_EASS(:)      ! heat flux between soil layer 1 and 2 (W/m2)
    real(r8), pointer :: forc_t(:)               ! atmospheric temperature [Kelvin]
!#urban    real(r8), pointer :: eflx_traffic(:)        ! traffic sensible heat flux (W/m**2)
!#urban    real(r8), pointer :: eflx_wasteheat(:)      ! sensible heat flux from urban heating/cooling sources of waste heat (W/m**2)
!#urban    real(r8), pointer :: eflx_wasteheat_pft(:)  ! sensible heat flux from urban heating/cooling sources of waste heat (W/m**2)
!#urban    real(r8), pointer :: eflx_heat_from_ac(:)   !sensible heat flux put back into canyon due to removal by AC (W/m**2)
!#urban    real(r8), pointer :: eflx_heat_from_ac_pft(:) !sensible heat flux put back into canyon due to removal by AC (W/m**2)
!#urban    real(r8), pointer :: eflx_traffic_pft(:)    ! traffic sensible heat flux (W/m**2)
!#urban    real(r8), pointer :: eflx_anthro(:)         ! total anthropogenic heat flux (W/m**2)
!#urban    real(r8), pointer :: canyon_hwr(:)          ! urban canyon height to width ratio
!#urban    real(r8), pointer :: wtlunit_roof(:)        ! weight of roof with respect to landunit
    real(r8), pointer :: eflx_bot_EASS(:)   ! heat flux from beneath column (W/m**2) [+ = upward]
    ...
    real(r8), pointer :: t_grnd_bef(:)          ! ground surface temperature [K]
! 
...
!
    real(r8), pointer :: eflx_gnet_EASS(:)       ! net ground heat flux into the surface (W/m**2)
    real(r8), pointer :: dgnetdT_EASS(:)         ! temperature derivative of ground net heat flux
!#urban    real(r8), pointer :: eflx_building_heat(:) ! heat flux from urban building interior to walls, roof (W/m**2)

! variables needed for SNICAR
    real(r8), pointer :: sabg_lyr(:,:)           ! absorbed solar radiation (pft,lyr) [W/m2]
    real(r8), pointer :: h2osno_EASS(:)          ! total snow water (col) [kg/m2]
    real(r8), pointer :: h2osoi_liq_EASS(:,:)    ! liquid water (col,lyr) [kg/m2]
    real(r8), pointer :: h2osoi_ice_EASS(:,:)    ! ice content (col,lyr) [kg/m2]

! Urban building HAC fluxes
!#urban    real(r8), pointer :: eflx_urban_ac(:)      ! urban air conditioning flux (W/m**2)
!#urban    real(r8), pointer :: eflx_urban_heat(:)    ! urban heating flux (W/m**2)
!
!
...
!#urban    logical  :: cool_on(lbl:ubl)                   ! is urban air conditioning on?
!#urban    logical  :: heat_on(lbl:ubl)                   ! is urban heating on?
    real(r8) :: hvap_EASS             ! latent heat of vaporization, depanding on temperature[J kg-1]
    logical  :: EASS_on = .false.                  ! is TridiagonalMod on
!-----------------------------------------------------------------------
...
!#    wtlunit_roof   => clm3%g%l%wtlunit_roof
    ...
    hc_soi_EASS    => clm3%g%l%c%ces%hc_soi_EASS
    hc_soisno_EASS => clm3%g%l%c%ces%hc_soisno_EASS
    eflx_fgr12_EASS=> clm3%g%l%c%cef%eflx_fgr12_EASS
...
!#    eflx_building_heat => clm3%g%l%c%cef%eflx_building_heat
    tssbef_EASS    => clm3%g%l%c%ces%tssbef_EASS
...
!#    eflx_urban_heat    => clm3%g%l%c%cef%eflx_urban_heat
    ...
    t_grnd_bef     => clm3%g%l%c%ces%t_grnd_bef
...
!#    qflx_tran_veg  => clm3%g%l%c%p%pwf%qflx_tran_veg
    eflx_gnet_EASS      => clm3%g%l%c%p%pef%eflx_gnet_EASS
    dgnetdT_EASS        => clm3%g%l%c%p%pef%dgnetdT_EASS
...
!#    eflx_anthro           => clm3%g%l%c%p%pef%eflx_anthro
    ...
    call SoilThermProp_EASS(lbc, ubc, num_nolakec, filter_nolakec, tk, cv)
...
!          lwrad_emit(c)  =       emg(c) * sb * t_grnd_EASS(c)**4
!          dlwrad_emit(c) = 4._r8*emg(c) * sb * t_grnd_EASS(c)**3
!       else
!          lwrad_emit(c)  =       emg(c) * sb * t_grnd_bef(c)**4
!          dlwrad_emit(c) = 4._r8*emg(c) * sb * t_grnd_bef(c)**3
!       end if
!    end do

! Revised by Shaobo Sun
! 2016/08/30
             ...
             if (pwtgcell(p)>0._r8 .or. ltype(l)==istice_mec) then
                if (ltype(l) /= isturb) then
                   if(forc_t(g) >= tfrz) then
                     eflx_gnet_EASS(p) = sabg(p) + dlrad(p) + (1-frac_veg_nosno(p))*emg(c)*forc_lwrad(g) - lwrad_emit(c) &
                                       - (eflx_sh_grnd(p)+qflx_evap_soi(p)*htvp(c))
                   else
                     eflx_gnet_EASS(p) = sabg(p) + dlrad(p) + (1-frac_veg_nosno(p))*emg(c)*forc_lwrad(g) - lwrad_emit(c) &
                                       - (eflx_sh_grnd(p)*0.60_r8 + qflx_evap_soi(p)*htvp(c)) 

! Site-level used
!                      hvap_EASS = LHvaporFunc_EASS( t_grnd_EASS(c) )
!                      eflx_gnet_EASS(p) = swnet_grnd_EASS(p) + lwrad_grnd_EASS(p) &
!                                           - (eflx_sh_grnd_EASS(p)*0.6_r8+qflx_evap_soi_EASS(p)*hvap_EASS)
                   end if

!                else
                   ! For urban columns we use the net longwave radiation
                   ! (eflx_lwrad_net) because of 
...
!                   eflx_anthro(p) = eflx_wasteheat_pft(p) + eflx_traffic_pft(p)
                ...
                dgnetdT_EASS(p) = - cgrnd_EASS(p) - dlwrad_emit(c)
                hs(c) = hs(c) + eflx_gnet_EASS(p) * pwtcol(p)
                dhsdT(c) = dhsdT(c) + dgnetdT_EASS(p) * pwtcol(p)
             end if
...
! Revised by Shaobo Sun, 2016/08/30
             ...
             if (pwtgcell(p)>0._r8 .or. ltype(l)==istice_mec) then
                ...
                if (ltype(l) /= isturb ) then
                    if(forc_t(g) >= tfrz) then
                       ...
                       eflx_gnet_top = sabg_lyr(p,lyr_top) + dlrad(p) + (1-frac_veg_nosno(p))*emg(c)*forc_lwrad(g) &
                                     - lwrad_emit(c) - (eflx_sh_grnd(p)*0.60_r8 + qflx_evap_soi(p)*htvp(c))

! Site-level used
!                        hvap_EASS = LHvaporFunc_EASS(t_grnd_EASS(c))
!                        eflx_gnet_top = sabg_lyr(p,lyr_top) + lwrad_grnd_EASS(p) &
!                                      - ( eflx_sh_grnd_EASS(p)*0.6_r8 + qflx_evap_soi_EASS(p)*hvap_EASS)
                    end if
...
!#                   else
                   fact(c,j) = dtime/cv(c,j) * dz_EASS(c,j) / (0.5_r8*(z_EASS(c,j)-zi_EASS(c,j-1)+capr*(z_EASS(c,j+1)-zi_EASS(c,j-1))))
!#                   end if
                   fn(c,j) = tk(c,j)*(t_soisno_EASS(c,j+1)-t_soisno_EASS(c,j))/(z_EASS(c,j+1)-z_EASS(c,j))
                else if (j <= nlevgrnd-1) then
                   fact(c,j) = dtime/cv(c,j)
                   fn(c,j) = tk(c,j)*(t_soisno_EASS(c,j+1)-t_soisno_EASS(c,j))/(z_EASS(c,j+1)-z_EASS(c,j))
                   dzm     = (z_EASS(c,j)-z_EASS(c,j-1))
...
!#                if (ctype(c)==icol_sunwall .or. ctype(c)==icol_shadewall .or. ctype(c)==icol_roof) then
!#                   fn(c,j) = tk(c,j) * (t_building(l) - cnfac*t_soisno(c,j))/(zi_EASS(c,j) - z_EASS(c,j))
!#                else
                   fn(c,j) = eflx_bot_EASS(c)
!#                end if
                end if
             end if  !if (j >= snl_EASS(c)+1)
          ...
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
             if (j >= snl_EASS(c)+1) then
                if (j == snl_EASS(c)+1) then
                   ...
                   if (j <= 1) then
                      rt(c,j) = t_soisno_EASS(c,j) + cnfac*fact(c,j)*( fn(c,j) - fn(c,j-1) ) + (fact(c,j)*sabg_lyr_col(c,j))
                   else
                      rt(c,j) = t_soisno_EASS(c,j) + cnfac*fact(c,j)*( fn(c,j) - fn(c,j-1) )
                   end if
...
!#                else
                      ...
                      ct(c,j) = 0._r8
                      rt(c,j) = t_soisno_EASS(c,j) - cnfac*fact(c,j)*fn(c,j-1) + fact(c,j)*fn(c,j)
...
!#                end if
      ...
      !#    else
      !#        at(c,j) = 1._r8     ! aviod NaN in cols excluding istsoil and istcrop
      !#        bt(c,j) = 1._r8     ! aviod NaN in cols excluding istsoil and istcrop
      !#        ct(c,j) = 1._r8     ! aviod NaN in cols excluding istsoil and istcrop
      !#        rt(c,j) = 1._r8     ! aviod NaN in cols excluding istsoil and istcrop
          end if      !# if (ltype(l) == istsoil .or. ltype(l) == istcrop)
    ...
    do fc = 1,num_nolakec
       ...
       l = clandunit(c)
       if ( ltype(l) == istsoil .or. ltype(l) == istcrop)  then
          call Tridiagonal_EASS( -nlevsno+1, nlevgrnd, jtop(c), c, & 
                          at(c,-nlevsno+1:nlevgrnd), bt(c,-nlevsno+1:nlevgrnd), &
                          ct(c,-nlevsno+1:nlevgrnd), rt(c,-nlevsno+1:nlevgrnd), &
                          t_soisno_EASS(c,-nlevsno+1:nlevgrnd))
       else
          do j=-nlevsno+1,nlevgrnd
...
!#          c = filter_nolakec(fc)
!#          l = clandunit(c)
!#          if (ltype(l) /= istsoil .AND. ltype(l) /= istcrop) then
!#              t_soisno_EASS(c,j) = t_soisno(c,j)
!#          end if 
!#      end do
!#    end do
          ...
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
             if (j >= snl_EASS(c)+1) then
                if (j <= nlevgrnd-1) then
                   fn1(c,j) = tk(c,j)*(t_soisno_EASS(c,j+1)-t_soisno_EASS(c,j))/(z_EASS(c,j+1)-z_EASS(c,j))
                else if (j == nlevgrnd) then
!#                ! For urban sunwall, shadewall, and roof columns, there is a non-zero heat flux across
!#                ! the bottom "soil" layer and the equations are derived assuming a prescribed internal
!#                ! building temperature. (See Oleson urban notes of 6/18/03).
...
!#    end do
    ...
    call PhaseChange_EASS (lbc, ubc, num_nolakec, filter_nolakec, fact, brr, hs, dhsdT, xmf, hs_top, sabg_lyr_col)

    do fc = 1,num_nolakec
       c = filter_nolakec(fc)
       l = clandunit(c)
       if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
          t_grnd_EASS(c) = t_soisno_EASS(c,snl_EASS(c)+1)
       else 
...
! Initialize soil heat content
    do fc = 1,num_nolakec
       ...
       if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
!#       if (ltype(l) /= isturb) then
          hc_soisno_EASS(c) = 0._r8
          hc_soi_EASS(c)    = 0._r8
!#       end if
          eflx_fgr12_EASS(c)= 0._r8
       end if
    end do
...
! Calculate soil heat content and soil plus snow heat content
    do j = -nlevsno+1,nlevgrnd
       do fc = 1,num_nolakec
          ...
          l = clandunit(c)
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
             eflx_fgr12_EASS(c) = -cnfac*fn(c,1) - (1._r8-cnfac)*fn1(c,1)
          else 
             eflx_fgr12_EASS(c) = eflx_fgr12(c)
          end if
        !#  if (ltype(l) /= isturb) then
          if (j >= snl_EASS(c)+1) then
             if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
                hc_soisno_EASS(c) = hc_soisno_EASS(c) + cv(c,j)*t_soisno_EASS(c,j) / 1.e6_r8
             else
                hc_soisno_EASS(c) = hc_soisno(c)
             end if 
          end if
          if (j >= 1) then
             if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
               hc_soi_EASS(c) = hc_soi_EASS(c) + cv(c,j)*t_soisno_EASS(c,j) / 1.e6_r8
             else
               hc_soi_EASS(c) = hc_soi(c) 
             end if
          end if
       end do
    end do

  end subroutine SoilTemper_EASS

!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: SoilThermProp_EASS
!
! !INTERFACE:
  subroutine SoilThermProp_EASS (lbc, ubc,  num_nolakec, filter_nolakec, tk, cv)
!
! !DESCRIPTION:
...
! !USES:
    ...
    use clm_varcon  , only : denh2o, denice, tfrz, tkwat, tkice, tkair, &
                             ...
                             icol_roof, icol_sunwall, icol_shadewall, &
                             icol_road_perv, icol_road_imperv, istcrop, istsoil
...
!
! !CALLED FROM:
! subroutine SoilTemperature_EASS in this module
!
! !REVISION HISTORY:
! 15 September 1999: Yongjiu Dai; Initial code
! 15 December 1999:  Paul Houser and Jon Radakovich; F90 Revision
...
!
    ...
    real(r8), pointer :: tksat0(:,:)      ! parameter for caculating dksat, added by Shaobo Sun
...
!#urban    real(r8), pointer :: tk_wall(:,:)     ! thermal conductivity of urban wall
!#urban    real(r8), pointer :: tk_roof(:,:)     ! thermal conductivity of urban roof
!#urban    real(r8), pointer :: tk_improad(:,:)  ! thermal conductivity of urban impervious road
!#urban    real(r8), pointer :: cv_wall(:,:)     ! thermal conductivity of urban wall
!#urban    real(r8), pointer :: cv_roof(:,:)     ! thermal conductivity of urban roof
!#urban    real(r8), pointer :: cv_improad(:,:)  ! thermal conductivity of urban impervious road
!#urban    integer,  pointer :: nlev_improad(:)  ! number of impervious road layers
...
!
    ...
    real(r8) :: fwat                      ! temp variable
    real(r8) :: fice                      ! temp variable
...
!-----------------------------------------------------------------------
    ...
    h2osoi_liq_EASS => clm3%g%l%c%cws%h2osoi_liq_EASS
    h2osoi_ice_EASS => clm3%g%l%c%cws%h2osoi_ice_EASS
!#urban    tk_wall    => clm3%g%l%lps%tk_wall
!#urban    tk_roof    => clm3%g%l%lps%tk_roof
!#urban    tk_improad => clm3%g%l%lps%tk_improad
!#urban    cv_wall    => clm3%g%l%lps%cv_wall
!#urban    cv_roof    => clm3%g%l%lps%cv_roof
!#urban    cv_improad => clm3%g%l%lps%cv_improad
!#urban    nlev_improad => clm3%g%l%lps%nlev_improad
    !-------------------------------
    ...
    do j = -nlevsno+1,nlevgrnd
       do fc = 1, num_nolakec
          ...
          if (j >= 1) then    
!#urban             l = clandunit(c)
!#urban             if (ctype(c) == icol_sunwall .OR. ctype(c) == icol_shadewall) then
!#urban                thk(c,j) = tk_wall(l,j)
!#urban             else if (ctype(c) == icol_roof) then
!#urban                thk(c,j) = tk_roof(l,j)
!#urban             else if (ctype(c) == icol_road_imperv .and. j >= 1 .and. j <= nlev_improad(l)) then
!#urban                thk(c,j) = tk_improad(l,j)
!#urban             elseif (ltype(l) /= istwet .AND. ltype(l) /= istice  &
!#urban                                        .AND. ltype(l) /= istice_mec) then
             if (ltype(l) == istcrop .or. ltype(l) == istsoil ) then  
                satw = (h2osoi_liq_EASS(c,j)/denh2o + h2osoi_ice_EASS(c,j)/denice)/(dz_EASS(c,j)*watsat(c,j))
                satw = min(1._r8, satw)

! Revised by Shaobo Sun, 2016/08/21
! Ref. Chadburn et al. 2015
 
                

                if (satw > .1e-6_r8) then
                   fl = h2osoi_liq_EASS(c,j)/(h2osoi_ice_EASS(c,j)+h2osoi_liq_EASS(c,j))
                   fwat = (h2osoi_liq_EASS(c,j)/(denh2o*dz_EASS(c,j))) &
                        / (h2osoi_liq_EASS(c,j)/(denh2o*dz_EASS(c,j)) + h2osoi_ice_EASS(c,j)/(denice*dz_EASS(c,j)))
                   fice = (h2osoi_ice_EASS(c,j)/(denice*dz_EASS(c,j))) &
                        / (h2osoi_liq_EASS(c,j)/(denh2o*dz_EASS(c,j)) + h2osoi_ice_EASS(c,j)/(denice*dz_EASS(c,j)))

                   if (t_soisno_EASS(c,j) >= tfrz) then       ! Unfrozen soil
                      dke = max(0._r8, log10(satw) + 1._r8)
                   else                               ! Frozen soil
                      dke = satw
                   end if  

                   dksat = tksat0(c,j)*(0.6_r8**(fwat*watsat(c,j)))*(2.29_r8**(4.0_r8*fice*watsat(c,j)))/(0.249_r8**watsat(c,j))
...
!#urban             else if (ltype(l) == istice .OR. ltype(l) == istice_mec) then
!#urban                thk(c,j) = tkwat
!#urban                if (t_soisno(c,j) < tfrz) thk(c,j) = tkice
!#urban             else if (ltype(l) == istwet) then                         
!#urban                if (j > nlevsoi) then 
!#urban                   thk(c,j) = thk_bedrock
!#urban                else
!#urban                   thk(c,j) = tkwat
!#urban                   if (t_soisno(c,j) < tfrz) thk(c,j) = tkice
!#urban                endif
             ...
             ! Only examine levels from snl_EASS(c)+1 -> 0 where snl_EASS(c) < 1

          if (ltype(l) == istcrop .or. ltype(l) == istsoil ) then
             if (snl_EASS(c)+1 < 1 .AND. (j >= snl_EASS(c)+1) .AND. (j <= 0)) then  
                bw = (h2osoi_ice_EASS(c,j)+h2osoi_liq_EASS(c,j))/dz_EASS(c,j)
!                thk(c,j) = tkair + (7.75e-5_r8 *bw + 1.105e-6_r8*bw*bw)*(tkice-tkair)

                thk(c,j) = 2.576e-6_r8*bw*bw + 0.074                                                ! add by Shaobo Sun, 2016/08/18, from EASS
             end if
          end if
    ...
    do j = -nlevsno+1,nlevgrnd
       do fc = 1,num_nolakec
          ...
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
             ...
             if (j >= snl_EASS(c)+1 .AND. j <= nlevgrnd-1) then
                 tk(c,j) = thk(c,j)*thk(c,j+1)*(z_EASS(c,j+1)-z_EASS(c,j)) &
                         /(thk(c,j)*(z_EASS(c,j+1)-zi_EASS(c,j))+thk(c,j+1)*(zi_EASS(c,j)-z_EASS(c,j)))
             ...
             ! the bottom "soil" layer and the equations are derived assuming a prescribed internal
             ! building temperature. (See Oleson urban notes of 6/18/03).
!#urban             if (ctype(c)==icol_sunwall .OR. ctype(c)==icol_shadewall .OR. ctype(c)==icol_roof) then
!#urban                tk(c,j) = thk(c,j)
!#urban             else
                 tk(c,j) = 0._r8
!#urban             end if
             end if
    ...
    do j = 1, nlevgrnd
       do fc = 1,num_nolakec
          ...
          l = clandunit(c)
!#urban          if (ctype(c)==icol_sunwall .OR. ctype(c)==icol_shadewall) then
!#urban             cv(c,j) = cv_wall(l,j) * dz(c,j)
!#urban          else if (ctype(c) == icol_roof) then
!#urban             cv(c,j) = cv_roof(l,j) * dz(c,j)
!#urban          else if (ctype(c) == icol_road_imperv .and. j >= 1 .and. j <= nlev_improad(l)) then
!#urban             cv(c,j) = cv_improad(l,j) * dz(c,j)
!#urban          elseif (ltype(l) /= istwet .AND. ltype(l) /= istice  &
!#urban                                     .AND. ltype(l) /= istice_mec) then
          if (ltype(l) == istcrop .or. ltype(l) == istsoil ) then
             cv(c,j) = csol(c,j) * ( 1 - watsat(c,j) ) * dz_EASS(c,j) + ( h2osoi_ice_EASS(c,j)*cpice &
                     + h2osoi_liq_EASS(c,j)*cpliq )
!#urban          else if (ltype(l) == istwet) then 
!#urban             cv(c,j) = (h2osoi_ice(c,j)*cpice + h2osoi_liq(c,j)*cpliq)
!#urban             if (j > nlevsoi) cv(c,j) = csol(c,j)*dz_EASS(c,j)
!#urban          else if (ltype(l) == istice .OR. ltype(l) == istice_mec) then 
!#urban             cv(c,j) = (h2osoi_ice(c,j)*cpice + h2osoi_liq(c,j)*cpliq)
!#urban          end if
             if (j == 1) then
                 if (snl_EASS(c)+1 == 1 .AND. h2osno_EASS(c) > 0._r8) then
                    cv(c,j) = cv(c,j) + cpice*h2osno_EASS(c)
                 end if
             end if
          end if
    ...
    do j = -nlevsno+1,0
       do fc = 1,num_nolakec
          ...
          l = clandunit(c)
          if (ltype(l) == istcrop .or. ltype(l) == istsoil ) then
             if (snl_EASS(c)+1 < 1 .and. j >= snl_EASS(c)+1) then
                cv(c,j) = cpliq*h2osoi_liq_EASS(c,j) + cpice*h2osoi_ice_EASS(c,j)
             end if
          end if
       end do
    end do

  end subroutine SoilThermProp_EASS

!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: PhaseChange_EASS
!
! !INTERFACE:
  subroutine PhaseChange_EASS (lbc, ubc, num_nolakec, filter_nolakec, fact, &
...
!
! !CALLED FROM:
! subroutine SoilTemper_EASS in this module
!
! !REVISION HISTORY:
! 15 September 1999: Yongjiu Dai; Initial code
! 15 December 1999:  Paul Houser and Jon Radakovich; F90 Revision
...
!
    ...
    real(r8), pointer :: eflx_snomelt_EASS(:)  !snow melt heat flux (W/m**2)
!#    real(r8), pointer :: eflx_snomelt_u_EASS(:) !urban snow melt heat flux (W/m**2)
...
! local pointers to original implicit in arrays
!
    real(r8), pointer :: h2osoi_liq_EASS(:,:)  !liquid water (kg/m2) (new)
    real(r8), pointer :: h2osoi_ice_EASS(:,:)  !ice lens (kg/m2) (new)
    real(r8), pointer :: tssbef_EASS(:,:)      !temperature at previous time step [K]
...
!
    integer, pointer :: imelt_EASS(:,:)        !flag for melting (=1), freezing (=2), Not=0 (new)
!
! !OTHER LOCAL VARIABLES:
!EOP
!
...
!#    eflx_snomelt_u_EASS => clm3%g%l%c%cef%eflx_snomelt_u_EASS
    eflx_snomelt_r_EASS  => clm3%g%l%c%cef%eflx_snomelt_r_EASS
    h2osoi_liq_EASS      => clm3%g%l%c%cws%h2osoi_liq_EASS
    h2osoi_ice_EASS      => clm3%g%l%c%cws%h2osoi_ice_EASS
    imelt_EASS           => clm3%g%l%c%cps%imelt_EASS
    t_soisno_EASS        => clm3%g%l%c%ces%t_soisno_EASS
    tssbef_EASS          => clm3%g%l%c%ces%tssbef_EASS
    ...
    clandunit            => clm3%g%l%c%landunit
    ltype                => clm3%g%l%itype
    qflx_snofrz_lyr_EASS => clm3%g%l%c%cwf%qflx_snofrz_lyr_EASS
    ...
    do fc = 1,num_nolakec
       ...
       l = clandunit(c) 
       if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
           qflx_snomelt_EASS(c) = 0._r8
           xmf(c) = 0._r8
           qflx_snofrz_lyr_EASS(c,-nlevsno+1:0) = 0._r8
       end if
    end do
       ...
       do fc = 1,num_nolakec
          ...
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
             if (j >= snl_EASS(c)+1) then
                 ...
                 hm(c,j) = 0._r8
                 xm(c,j) = 0._r8
                 wice0(c,j) = h2osoi_ice_EASS(c,j)
                 wliq0(c,j) = h2osoi_liq_EASS(c,j)
                 wmass0(c,j) = h2osoi_ice_EASS(c,j) + h2osoi_liq_EASS(c,j)
       ...
       do fc = 1,num_nolakec
          ...
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
             if (j >= snl_EASS(c)+1) then
                ...
                if (h2osoi_ice_EASS(c,j) > 0._r8 .AND. t_soisno_EASS(c,j) > tfrz) then
                   imelt_EASS(c,j) = 1
                   t_soisno_EASS(c,j) = tfrz
                end if

                ! Freezing identification
                ! If liquid exists below melt point, freeze some to ice.
                if (h2osoi_liq_EASS(c,j) > 0._r8 .AND. t_soisno_EASS(c,j) < tfrz) then
                   imelt_EASS(c,j) = 2
                   t_soisno_EASS(c,j) = tfrz
                end if
             end if
    ...
    do j = 1,nlevgrnd             ! soil layers 
       do fc = 1,num_nolakec
          ...
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
             ...
             if (h2osoi_ice_EASS(c,j) > 0. .AND. t_soisno_EASS(c,j) > tfrz) then
                imelt_EASS(c,j) = 1
                t_soisno_EASS(c,j) = tfrz
             end if
             ...
             if(t_soisno_EASS(c,j) < tfrz) then
                smp = hfus*(tfrz-t_soisno_EASS(c,j))/(grav*t_soisno_EASS(c,j)) * 1000._r8  !(mm)
                supercool(c,j) = watsat(c,j)*(smp/sucsat(c,j))**(-1._r8/bsw(c,j))
                supercool(c,j) = supercool(c,j)*dz_EASS(c,j)*1000._r8       ! (mm)
             end if

             if (h2osoi_liq_EASS(c,j) > supercool(c,j) .AND. t_soisno_EASS(c,j) < tfrz) then
                imelt_EASS(c,j) = 2
                t_soisno_EASS(c,j) = tfrz
             end if

             ! If snow exists, but its thickness is less than the critical value (0.01 m)
             if (snl_EASS(c)+1 == 1 .AND. h2osno_EASS(c) > 0._r8 .AND. j == 1) then
                if (t_soisno_EASS(c,j) > tfrz) then
                   imelt_EASS(c,j) = 1
                   t_soisno_EASS(c,j) = tfrz
                end if
    ...
    do j = -nlevsno+1,nlevgrnd       ! all layers
       do fc = 1,num_nolakec
          ...
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
             ...
             if (j >= snl_EASS(c)+1) then
                ...
                if (imelt_EASS(c,j) > 0) then
                   tinc = t_soisno_EASS(c,j)-tssbef_EASS(c,j)
                ...
                if (imelt_EASS(c,j) == 1 .AND. hm(c,j) < 0._r8) then
                   hm(c,j) = 0._r8
                   imelt_EASS(c,j) = 0
                end if
                if (imelt_EASS(c,j) == 2 .AND. hm(c,j) > 0._r8) then
                   hm(c,j) = 0._r8
                   imelt_EASS(c,j) = 0
                end if
                ...
                if (imelt_EASS(c,j) > 0 .and. abs(hm(c,j)) > 0._r8) then
                   ...
                   if (j == 1) then
                      if (snl_EASS(c)+1 == 1 .AND. h2osno_EASS(c) > 0._r8 .AND. xm(c,j) > 0._r8) then
                         temp1 = h2osno_EASS(c)                           ! kg/m2
                         h2osno_EASS(c) = max(0._r8,temp1-xm(c,j))
                         propor = h2osno_EASS(c)/temp1
                         snowdp_EASS(c) = propor * snowdp_EASS(c)
                         heatr = hm(c,j) - hfus*(temp1-h2osno_EASS(c))/dtime   ! W/m2
                         if (heatr > 0._r8) then
                         ...
                         qflx_snomelt_EASS(c) = max(0._r8,(temp1-h2osno_EASS(c)))/dtime   ! kg/(m2 s)
                         xmf(c) = hfus * qflx_snomelt_EASS(c)
                   ...
                   if (xm(c,j) > 0._r8) then
                      h2osoi_ice_EASS(c,j) = max(0._r8, wice0(c,j)-xm(c,j))
                      heatr = hm(c,j) - hfus*(wice0(c,j)-h2osoi_ice_EASS(c,j))/dtime
                   else if (xm(c,j) < 0._r8) then
                      if (j <= 0) then
                         h2osoi_ice_EASS(c,j) = min(wmass0(c,j), wice0(c,j)-xm(c,j))  ! snow
                      else
                         if (wmass0(c,j) < supercool(c,j)) then
                            h2osoi_ice_EASS(c,j) = 0._r8
                         else
                            h2osoi_ice_EASS(c,j) = min(wmass0(c,j) - supercool(c,j),wice0(c,j)-xm(c,j))
                         end if
                      end if
                      heatr = hm(c,j) - hfus*(wice0(c,j)-h2osoi_ice_EASS(c,j))/dtime
                   end if

                   h2osoi_liq_EASS(c,j) = max(0._r8,wmass0(c,j)-h2osoi_ice_EASS(c,j))

                   if (abs(heatr) > 0._r8) then
                      if (j > snl_EASS(c)+1) then
                         t_soisno_EASS(c,j) = t_soisno_EASS(c,j) + fact(c,j)*heatr
                      else
                         t_soisno_EASS(c,j) = t_soisno_EASS(c,j) + fact(c,j)*heatr/(1._r8-fact(c,j)*dhsdT(c))
                      end if
                      if (j <= 0) then    ! snow
                         if (h2osoi_liq_EASS(c,j)*h2osoi_ice_EASS(c,j)>0._r8) t_soisno_EASS(c,j) = tfrz
                      end if
                   end if

                   xmf(c) = xmf(c) + hfus * (wice0(c,j)-h2osoi_ice_EASS(c,j))/dtime

                   if (imelt_EASS(c,j) == 1 .AND. j < 1) then
                      qflx_snomelt_EASS(c) = qflx_snomelt_EASS(c) + max(0._r8,(wice0(c,j)-h2osoi_ice_EASS(c,j)))/dtime
                   end if

                   ! layer freezing mass flux (positive):
                   if (imelt_EASS(c,j) == 2 .AND. j < 1) then
                      qflx_snofrz_lyr_EASS(c,j) = max(0._r8,(h2osoi_ice_EASS(c,j)-wice0(c,j)))/dtime
                   end if
       ...
       !#      endif     ! istice_mec
              ...
              qflx_snofrz_lyr_EASS(c,j) = qflx_snofrz_lyr(c,j)
              t_soisno_EASS(c,j) = t_soisno(c,j)
              imelt_EASS(c,j) = imelt(c,j)              
              snowdp_EASS(c) = snowdp(c)

          end if  ! end of if ltype(l)
       end do     ! end of column-loop
    enddo         ! end of level-loop
    ...
    do fc = 1,num_nolakec
       ...
       l = clandunit(c)
       if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
            eflx_snomelt_EASS(c) = qflx_snomelt_EASS(c) * hfus
            eflx_snomelt_r_EASS(c) = eflx_snomelt_EASS(c)
       else 
            eflx_snomelt_EASS(c) = eflx_snomelt(c)
            eflx_snomelt_r_EASS(c) = eflx_snomelt_r(c)
            snowdp_EASS(c) = snowdp(c)
 ...
 !#      end if
  ...
  end subroutine PhaseChange_EASS


end module SoilTemper_EASSMod
