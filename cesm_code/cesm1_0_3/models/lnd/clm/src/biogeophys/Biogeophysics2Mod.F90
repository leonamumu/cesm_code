...
! !USES:
    ...
    use clm_varcon        , only : hvap, hsub, cpair, grav, vkc, tfrz, sb, &
    ...
    !----------------------------------
    ! added by Jing Chen, May 26,2012
    use PenmanEqu_EASSMod , only : LHvaporFunc_EASS
    use SoilTemper_EASSMod, only : SoilTemper_EASS
...
!
    ...
    real(r8), pointer :: qflx_trans_over_EASS(:)    ! transpiration from overstory[mmH2O s-1]
    real(r8), pointer :: qflx_trans_under_EASS(:)   ! transpiration from understory[mmH2O s-1]
    ...
    real(r8), pointer :: qflx_evap_over_EASS(:)     ! water(liquid) evaporation from overstroy[mmH2O s-1]
    real(r8), pointer :: qflx_evap_under_EASS(:)    ! water(liquid) evaporation from understory[mmH2O s-1]
    ...
    real(r8), pointer :: qflx_evap_grnd_EASS(:)     ! water(liquid) evaporation from ground surface[mmH2O s-1]
    real(r8), pointer :: qflx_evap_pond_EASS(:)     ! water(liquid) evaporation from pond[mmH2O s-1]
    real(r8), pointer :: qflx_evap_snow_over_EASS(:)  ! evaporation from snow of overstory[mmH2O s-1]
    real(r8), pointer :: qflx_evap_snow_under_EASS(:) ! evaporation from snow of understory[mmH2O s-1]
    ...
    real(r8), pointer :: forc_rho(:)                  ! air density [kg m-3]
    real(r8), pointer :: cpairg_EASS(:)               ! specific heat capacity of surface ground[J kg-1 K-1]
    real(r8), pointer :: ra_grnd_EASS(:)              ! air resistance of grnd [s m-1]
    real(r8), pointer :: lwnet_EASS(:)                ! net infrared (longwave) rad (W/m**2) [+ = to atm] 
...
! 
    ...
    real(r8), pointer :: eflx_lh_tot_EASS(:)     ! total latent heat flux [W m-2]
...
!
    ...
    real(r8) :: fact_EASS(lbc:ubc, -nlevsno+1:nlevgrnd) ! used in computing tridiagonal matrix
    real(r8) :: xmf_EASS(lbc:ubc)     ! total latent heat of phase change of ground water
    real(r8) :: tinc_EASS(lbc:ubc)    ! temperature difference of two time step
    real(r8) :: eg_EASS(lbp:ubp)      ! saturated vapor pressure of surface ground[pa], 
                                      ! depanding on forc_t[K] and forc_pbot[pa]
...
!-----------------------------------------------------------------------
    ...
    eflx_lh_tot_EASS          => clm3%g%l%c%p%pef%eflx_lh_tot_EASS
    qflx_evap_can_EASS        => clm3%g%l%c%p%pwf%qflx_evap_can_EASS
    qflx_evtr_sunlit_EASS     => clm3%g%l%c%p%pwf%qflx_evtr_sunlit_EASS
    qflx_evtr_shaded_EASS     => clm3%g%l%c%p%pwf%qflx_evtr_shaded_EASS
    qflx_evap_grnd_EASS       => clm3%g%l%c%p%pwf%qflx_evap_grnd_EASS
    eflx_lh_can_EASS          => clm3%g%l%c%p%pef%eflx_lh_can_EASS
    eflx_lh_over_EASS         => clm3%g%l%c%p%pef%eflx_lh_over_EASS
    eflx_lh_under_EASS        => clm3%g%l%c%p%pef%eflx_lh_under_EASS
    eflx_lh_sunlit_EASS       => clm3%g%l%c%p%pef%eflx_lh_sunlit_EASS
    eflx_lh_shaded_EASS       => clm3%g%l%c%p%pef%eflx_lh_shaded_EASS
    eflx_lh_grnd_EASS         => clm3%g%l%c%p%pef%eflx_lh_grnd_EASS
    eflx_sh_can_EASS          => clm3%g%l%c%p%pef%eflx_sh_can_EASS
    cpairg_EASS               => clm3%g%l%c%p%pes%cpairg_EASS
    ra_grnd_EASS              => clm3%g%l%c%p%pps%ra_grnd_EASS
    eflx_lh_veg               => clm3%g%l%c%p%pef%eflx_lh_veg
    eflx_lh_tot_u_EASS        => clm3%g%l%c%p%pef%eflx_lh_tot_u_EASS
    eflx_lh_tot_r_EASS        => clm3%g%l%c%p%pef%eflx_lh_tot_r_EASS
    eflx_sh_tot_u_EASS        => clm3%g%l%c%p%pef%eflx_sh_tot_u_EASS
    eflx_sh_tot_r_EASS        => clm3%g%l%c%p%pef%eflx_sh_tot_r_EASS
    eflx_sh_grnd_r_EASS       => clm3%g%l%c%p%pef%eflx_sh_grnd_r_EASS
    eflx_sh_grnd_u_EASS       => clm3%g%l%c%p%pef%eflx_sh_grnd_u_EASS
    lwnet_EASS                => clm3%g%l%c%p%pef%lwnet_EASS
    lwnet_r_EASS              => clm3%g%l%c%p%pef%lwnet_r_EASS
    lwnet_u_EASS              => clm3%g%l%c%p%pef%lwnet_u_EASS
    eflx_soil_grnd_EASS   => clm3%g%l%c%p%pef%eflx_soil_grnd_EASS
    eflx_soil_grnd_u_EASS => clm3%g%l%c%p%pef%eflx_soil_grnd_u_EASS
    eflx_soil_grnd_r_EASS => clm3%g%l%c%p%pef%eflx_soil_grnd_r_EASS
    swnet_grnd_EASS       => clm3%g%l%c%p%pef%swnet_grnd_EASS
    lwrad_grnd_EASS       => clm3%g%l%c%p%pef%lwrad_grnd_EASS
    qflx_sub_snow_EASS    => clm3%g%l%c%p%pwf%qflx_sub_snow_EASS
    qflx_dew_snow_EASS    => clm3%g%l%c%p%pwf%qflx_dew_snow_EASS
    qflx_dew_grnd_EASS    => clm3%g%l%c%p%pwf%qflx_dew_grnd_EASS
    qflx_snwcp_liq_EASS   => clm3%g%l%c%p%pwf%qflx_snwcp_liq_EASS
    qflx_snwcp_ice_EASS   => clm3%g%l%c%p%pwf%qflx_snwcp_ice_EASS
    qflx_tran_veg_EASS    => clm3%g%l%c%p%pwf%qflx_tran_veg_EASS
    ...
    ! istsoil and istcrop only
    call SoilTemper_EASS(lbl, ubl, lbc, ubc, num_urbanl, filter_urbanl, &
                             num_nolakec, filter_nolakec, xmf_EASS , fact_EASS)
    !--------------------------
    ...
    do fc = 1,num_nolakec
       ...
       tinc_EASS(c) = t_soisno_EASS(c,j) - tssbef_EASS(c,j)
    ...
    do fp = 1,num_nolakep
       ...
       !-------------------
       ! added by Jing Chen Oct 23 2012
       eflx_sh_grnd_EASS(p)  = eflx_sh_grnd_EASS(p) + tinc_EASS(c) * cgrnds_EASS(p)
       qflx_evap_soi_EASS(p) = qflx_evap_soi_EASS(p) + tinc_EASS(c) * cgrndl_EASS(p)
       !----------------
    end do
    ...
    do fp = 1,num_nolakep
       ...
       !-----------------------
       ! added by Jing Chen, June 9,2012
       ...
       if ( frac_veg_nosno(p) /= 0 .and. ( ltype(l) == istsoil .or. ltype(l) == istcrop) ) then
            eflx_sh_grnd_EASS(p) = ( t_grnd_EASS(c)-forc_t_col(c)) *forc_rho(g)*cpairg_EASS(p)/ra_grnd_EASS(p)
            qflx_evap_soi_EASS(p)= qflx_evap_soi_EASS(p)
       else
            eflx_sh_grnd_EASS(p) = eflx_sh_grnd(p)
            qflx_evap_soi_EASS(p)= qflx_evap_soi(p)
       end if
          !---------------------------
       ...
       if (ltype(l) /= isturb) then
          ...
          eflx_soil_grnd_EASS(p) = eflx_soil_grnd(p)
          if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
            eflx_soil_grnd_r(p) = eflx_soil_grnd(p)
            eflx_soil_grnd_r_EASS(p) = eflx_soil_grnd(p)
            !---------------------
            ! added by Jing Chen Oct 22 2012
            if (frac_veg_nosno(p) /= 0) then
                 hvap_EASS=LHvaporFunc_EASS(t_grnd_EASS(c))
                 eflx_soil_grnd_EASS(p)   = swnet_grnd_EASS(p) + lwrad_grnd_EASS(p) &
                                       - (eflx_sh_grnd_EASS(p) + qflx_evap_soi_EASS(p)*hvap_EASS ) 
                 eflx_soil_grnd_r_EASS(p) = eflx_soil_grnd_EASS(p) 
            end if
            !----------------------
          end if
       else
          ...
          eflx_soil_grnd_EASS(p)   = eflx_soil_grnd(p) 
          eflx_soil_grnd_u_EASS(p) = eflx_soil_grnd_u(p)
          !----------------
       end if
       ...
       !total water vapor flux , latent heat flux and sensible heat flux
       if ( frac_veg_nosno(p) /= 0 .and. ( ltype(l) == istsoil .or. ltype(l) == istcrop) ) then

            !(1) total water vapor flux
            qflx_evap_tot_EASS(p) = qflx_trans_over_EASS(p) + qflx_trans_under_EASS(p) &
                             + qflx_evap_over_EASS(p) + qflx_evap_under_EASS(p) &
                             + qflx_evap_snow_over_EASS(p) + qflx_evap_snow_under_EASS(p) &
                             + qflx_evap_soi_EASS(p)
                       !#      + qflx_evap_grnd_EASS(p) + qflx_evap_pond_EASS(p) &
                       !#      + qflx_evap_snow_grnd_EASS(p)

            qflx_evap_can_EASS(p) = qflx_trans_over_EASS(p) + qflx_trans_under_EASS(p) &
                             + qflx_evap_over_EASS(p) + qflx_evap_under_EASS(p) &
                             + qflx_evap_snow_over_EASS(p) + qflx_evap_snow_under_EASS(p) 

            qflx_evtr_sunlit_EASS(p) = qflx_trans_sunlit_EASS(p) + qflx_evap_sunlit_EASS(p) + qflx_evap_snow_sunlit_EASS(p)
            qflx_evtr_shaded_EASS(p) = qflx_trans_shaded_EASS(p) + qflx_evap_shaded_EASS(p) + qflx_evap_snow_shaded_EASS(p)

!#            qflx_evap_soi_EASS(p) = qflx_evap_grnd_EASS(p) + qflx_evap_pond_EASS(p) &
!#                              + qflx_evap_snow_grnd_EASS(p)  

            !(2)total latent heat flux
            hvap_EASS = LHvaporFunc_EASS(forc_t(g))
            eflx_lh_tot_EASS(p) = hvap_EASS *(qflx_trans_over_EASS(p) + qflx_trans_under_EASS(p) &
                           + qflx_evap_over_EASS(p) + qflx_evap_under_EASS(p) + qflx_evap_soi_EASS(p) ) &
                           + hsub * ( qflx_evap_snow_over_EASS(p) + qflx_evap_snow_under_EASS(p) ) 

            eflx_lh_sunlit_EASS(p) = hvap_EASS *(qflx_trans_sunlit_EASS(p) + qflx_evap_sunlit_EASS(p) ) &
                           + hsub * qflx_evap_snow_sunlit_EASS(p)
            eflx_lh_shaded_EASS(p) = hvap_EASS *(qflx_trans_shaded_EASS(p) + qflx_evap_shaded_EASS(p) ) &
                           + hsub * qflx_evap_snow_shaded_EASS(p)

            !(3)total sensible heat flux   
!#            eflx_sh_over_EASS(p)=max(-200.0_r8,eflx_sh_over_EASS(p))
!#            eflx_sh_under_EASS(p)=max(-200.0_r8,eflx_sh_under_EASS(p))
            eflx_sh_tot_EASS(p) = eflx_sh_over_EASS(p) + eflx_sh_under_EASS(p) + eflx_sh_grnd_EASS(p) 
            eflx_sh_tot_EASS(p) = max(-200.0_r8,eflx_sh_tot_EASS(p))
            eflx_sh_can_EASS(p) = eflx_sh_over_EASS(p) + eflx_sh_under_EASS(p)
        else
            eflx_sh_tot_EASS(p)   = eflx_sh_tot(p)
            qflx_evap_tot_EASS(p) = qflx_evap_tot(p)
            eflx_lh_tot_EASS(p)   = eflx_lh_tot(p)   
        end if
       !---------------------------------------
       ...
       if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
         ...
         !-------------------------------
         ! added by Jing Chen, Sep 8 2012
         eflx_lh_tot_u(p)=0._r8
         eflx_sh_tot_u(p)=0._r8
         if (frac_veg_nosno(p) /= 0) then
             eflx_lh_tot_r_EASS(p)= eflx_lh_tot_EASS(p)
             eflx_sh_tot_r_EASS(p)= eflx_sh_tot_EASS(p) 
             eflx_lh_tot_u_EASS(p)= 0._r8
             eflx_sh_tot_u_EASS(p)= 0._r8
         else
             eflx_lh_tot_r_EASS(p)= eflx_lh_tot(p)
             eflx_sh_tot_r_EASS(p)= eflx_sh_tot(p) 
             eflx_lh_tot_u_EASS(p)= 0._r8
             eflx_sh_tot_u_EASS(p)= 0._r8
         end if
         !-------------------------------
         ...
         eflx_lh_tot_u_EASS(p)= eflx_lh_tot_EASS(p)
         eflx_sh_tot_u_EASS(p)= eflx_sh_tot_EASS(p)
         eflx_lh_tot_r_EASS(p)= 0._r8
         eflx_sh_tot_r_EASS(p)= 0._r8
         !-------------------------------
       end if
       ...
       !-------------------
       ! added by Jing Chen Oct 22 2012
       qflx_evap_grnd_EASS(p) = 0._r8
       qflx_sub_snow_EASS(p) = 0._r8
       qflx_dew_snow_EASS(p) = 0._r8
       qflx_dew_grnd_EASS(p) = 0._r8
       !------------------
       ...
       if ( frac_veg_nosno(p) /= 0 .and. ( ltype(l) == istsoil .or. ltype(l) == istcrop) ) then
           ...
           if ( qflx_evap_soi_EASS(p) >= 0._r8 ) then
               ! for evaporation partitioning between liquid evap and ice sublimation, 
               ! use the ratio of liquid to (liquid+ice) in the top layer to determine split 
               if ( ( h2osoi_liq_EASS(c,j) + h2osoi_ice_EASS(c,j) ) > 0._r8 ) then
                   qflx_evap_grnd_EASS(p) = max( qflx_evap_grnd_EASS(p), 0._r8 )
               else
                   qflx_evap_grnd_EASS(p) = 0._r8
               end if
               qflx_sub_snow_EASS(p) = qflx_evap_snow_grnd_EASS(p)
           else
               if ( t_grnd_EASS(c) < tfrz ) then
                   qflx_dew_snow_EASS(p) = abs( qflx_evap_soi_EASS(p) )
               else
                   qflx_dew_grnd_EASS(p) = abs( qflx_evap_soi_EASS(p) )
               end if
           end if

           if (snl_EASS(c) < 0 .and. do_capsnow(c)) then
              qflx_snwcp_liq_EASS(p) = qflx_snwcp_liq_EASS(p) + qflx_dew_grnd_EASS(p)
              qflx_snwcp_ice_EASS(p) = qflx_snwcp_ice_EASS(p) + qflx_dew_snow_EASS(p)
           end if
       else
           qflx_evap_grnd_EASS(p) = qflx_evap_grnd(p) 
           qflx_sub_snow_EASS(p)  = qflx_sub_snow(p)
           qflx_dew_snow_EASS(p)  = qflx_dew_snow(p)      
           qflx_snwcp_liq_EASS(p) = qflx_snwcp_liq(p)
           qflx_snwcp_ice_EASS(p) = qflx_snwcp_ice(p)
       end if
       !-------------------
       ...
       eflx_lh_veg(p)    = eflx_lh_vege(p) + eflx_lh_vegt(p)   
       !---------------
       ! added by Jing Chen Oct 21 2012
       if ( frac_veg_nosno(p) /= 0 .and. ( ltype(l) == istsoil .or. ltype(l) == istcrop) ) then
             eflx_lh_can_EASS(p) = hvap_EASS *(qflx_trans_over_EASS(p) + qflx_trans_under_EASS(p) &
                           + qflx_evap_over_EASS(p) + qflx_evap_under_EASS(p)) &
                           + hsub * (qflx_evap_snow_over_EASS(p) + qflx_evap_snow_under_EASS(p) )

             eflx_lh_over_EASS(p) = hvap_EASS *(qflx_trans_over_EASS(p) + qflx_evap_over_EASS(p) ) &
                           + hsub * qflx_evap_snow_over_EASS(p)

             eflx_lh_under_EASS(p) = hvap_EASS *(qflx_trans_under_EASS(p) + qflx_evap_under_EASS(p)) &
                           + hsub * qflx_evap_snow_under_EASS(p)

             eflx_lh_grnd_EASS(p) = hvap_EASS *(qflx_evap_soi_EASS(p) )!#+ qflx_evap_pond_EASS(p)) &
  !#                          + hsub * qflx_evap_snow_grnd_EASS(p)
             qflx_tran_veg_EASS(p) = qflx_trans_over_EASS(p) + qflx_trans_under_EASS(p)
       else
             
             eflx_lh_can_EASS(p)   = eflx_lh_veg(p)
             eflx_lh_over_EASS(p)  = eflx_lh_veg(p)
             eflx_lh_under_EASS(p) = 0._r8
             eflx_lh_grnd_EASS(p)  = eflx_lh_grnd(p)
             qflx_tran_veg_EASS(p) = qflx_tran_veg(p)
       end if
    ...
    do fp = 1,num_nolakep
       ...
       if (ltype(l) /= isturb) then
          ...
          !----------------------------------
          ! added by Jing Chen, Sep 8 2012
          lwnet_EASS(p)=-eflx_lwrad_net(p)
          lwnet_u_EASS(p)=lwnet_EASS(p)
          !------------------------------------
       end if
    end do
