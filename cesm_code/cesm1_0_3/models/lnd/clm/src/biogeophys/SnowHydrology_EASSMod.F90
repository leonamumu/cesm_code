...
! !PUBLIC MEMBER FUNCTIONS:
  public :: SnowWater_EASS         ! Change of snow mass and the snow water onto soil
  public :: SnowCompaction_EASS    ! Change in snow layer thickness due to compaction
  public :: CombineSnowLayers_EASS ! Combine snow layers less than a min thickness
  public :: DivideSnowLayers_EASS  ! Subdivide snow layers if they exceed maximum thickness
  public :: BuildSnowFilter_EASS   ! Construct snow/no-snow filters
!
...
!BOP
!
! !IROUTINE: SnowWater_EASS
!
! !INTERFACE:
  subroutine SnowWater_EASS(lbc, ubc, num_snowc_EASS, filter_snowc_EASS, &
...
! !ARGUMENTS:
    ...
    integer, intent(in) :: num_snowc_EASS                   ! number of snow points in column filter
    integer, intent(in) :: filter_snowc_EASS(ubc-lbc+1)     ! column filter for snow points
    integer, intent(in) :: num_nosnowc_EASS                 ! number of non-snow points in column filter
    integer, intent(in) :: filter_nosnowc_EASS(ubc-lbc+1)   ! column filter for non-snow points
!
! !CALLED FROM:
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------
    ...
    qflx_rain_grnd_EASS => clm3%g%l%c%cwf%pwf_a%qflx_rain_grnd_EASS
...
!#    forc_aer         => clm_a2l%forc_aer
    ...
    do fc = 1,num_snowc_EASS
       ...
       if (do_capsnow(c)) then
          wgdif = h2osoi_ice_EASS(c,snl_EASS(c)+1) - qflx_sub_snow_EASS(c)*dtime
          h2osoi_ice_EASS(c,snl_EASS(c)+1) = wgdif
          if (wgdif < 0._r8) then
             h2osoi_ice_EASS(c,snl_EASS(c)+1) = 0._r8
             h2osoi_liq_EASS(c,snl_EASS(c)+1) = h2osoi_liq_EASS(c,snl_EASS(c)+1) + wgdif
          end if
          h2osoi_liq_EASS(c,snl_EASS(c)+1) = h2osoi_liq_EASS(c,snl_EASS(c)+1) - qflx_evap_grnd_EASS(c) * dtime
       else
          wgdif = h2osoi_ice_EASS(c,snl_EASS(c)+1) + (qflx_dew_snow_EASS(c) - qflx_sub_snow_EASS(c)) * dtime
          h2osoi_ice_EASS(c,snl_EASS(c)+1) = wgdif
          if (wgdif < 0._r8) then
             h2osoi_ice_EASS(c,snl_EASS(c)+1) = 0._r8
             h2osoi_liq_EASS(c,snl_EASS(c)+1) = h2osoi_liq_EASS(c,snl_EASS(c)+1) + wgdif
          end if
          h2osoi_liq_EASS(c,snl_EASS(c)+1) = h2osoi_liq_EASS(c,snl_EASS(c)+1) +  &
               (qflx_rain_grnd_EASS(c) + qflx_dew_grnd_EASS(c) - qflx_evap_grnd_EASS(c)) * dtime
       end if
       h2osoi_liq_EASS(c,snl_EASS(c)+1) = max(0._r8, h2osoi_liq_EASS(c,snl_EASS(c)+1))
    end do
    ...
    do j = -nlevsno+1, 0
       do fc = 1, num_snowc_EASS
          ...
          if (j >= snl_EASS(c)+1) then
             vol_ice(c,j) = min(1._r8, h2osoi_ice_EASS(c,j)/(dz_EASS(c,j)*denice))
             eff_porosity(c,j) = 1._r8 - vol_ice(c,j)
             vol_liq(c,j) = min(eff_porosity(c,j),h2osoi_liq_EASS(c,j)/(dz_EASS(c,j)*denh2o))
          end if
       end do
    end do
...
!#    qin_dst4(:)   = 0._r8
    ...
    do j = -nlevsno+1, 0
       do fc = 1, num_snowc_EASS
          c = filter_snowc_EASS(fc)
          if (j >= snl_EASS(c)+1) then
             h2osoi_liq_EASS(c,j) = h2osoi_liq_EASS(c,j) + qin(c)
...
!#             mss_dst4(c,j)  = mss_dst4(c,j) + qin_dst4(c)
             ...
             if (j <= -1) then
                ...
                if (eff_porosity(c,j) < wimp .OR. eff_porosity(c,j+1) < wimp) then
                   ...
                   qout(c) = max(0._r8,(vol_liq(c,j)-ssi*eff_porosity(c,j))*dz_EASS(c,j))
                   qout(c) = min(qout(c),(1._r8-vol_ice(c,j+1)-vol_liq(c,j+1))*dz_EASS(c,j+1))
                end if
             else
                qout(c) = max(0._r8,(vol_liq(c,j) - ssi*eff_porosity(c,j))*dz_EASS(c,j))
             end if
             qout(c) = qout(c)*1000._r8
             h2osoi_liq_EASS(c,j) = h2osoi_liq_EASS(c,j) - qout(c)
             qin(c) = qout(c)
...
!#             qin_dst4(c) = qout_dst4(c)
    ...
    do j = -nlevsno+1, 0
       do fc = 1, num_snowc_EASS
          c = filter_snowc_EASS(fc)
          if (j >= snl_EASS(c)+1) then
              dz_EASS(c,j) = max(dz_EASS(c,j),h2osoi_liq_EASS(c,j)/denh2o + h2osoi_ice_EASS(c,j)/denice)
          end if
       end do
    end do
    ...
    do fc = 1, num_snowc_EASS
       ...
       qflx_top_soil_EASS(c) = qout(c) / dtime
    end do

    do fc = 1, num_nosnowc_EASS
       c = filter_nosnowc_EASS(fc)
       qflx_top_soil_EASS(c) = qflx_rain_grnd_EASS(c) + qflx_snomelt_EASS(c)
    end do
...
!#    end do
  ...
  end subroutine SnowWater_EASS

!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: SnowCompaction_EASS
!
! !INTERFACE:
  subroutine SnowCompaction_EASS(lbc, ubc, num_snowc_EASS, filter_snowc_EASS)
!
! !DESCRIPTION:
...
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: lbc, ubc                ! column bounds
    integer, intent(in) :: num_snowc_EASS               ! number of column snow points in column filter
    integer, intent(in) :: filter_snowc_EASS(ubc-lbc+1) ! column filter for snow points
!
! !CALLED FROM:
...
! local pointers to implicit in arguments
!
    integer,  pointer :: imelt_EASS(:,:)        !flag for melting (=1), freezing (=2), Not=0
...
!-----------------------------------------------------------------------
    ...
    do j = -nlevsno+1, 0
       do fc = 1, num_snowc_EASS
          ...
          if (j >= snl_EASS(c)+1) then
             ...
             wx = h2osoi_ice_EASS(c,j) + h2osoi_liq_EASS(c,j)
             void = 1._r8 - (h2osoi_ice_EASS(c,j)/denice + h2osoi_liq_EASS(c,j)/denh2o) / dz_EASS(c,j)
             ...
             if (ltype(l)==istice_mec .and. void < 0._r8) then
                dz_EASS(c,j) = h2osoi_ice_EASS(c,j)/denice + h2osoi_liq_EASS(c,j)/denh2o
                void = 0._r8
             endif

             ! Allow compaction only for non-saturated node and higher ice lens node.
             if (void > 0.001_r8 .and. h2osoi_ice_EASS(c,j) > .1_r8) then
                bi = h2osoi_ice_EASS(c,j) / dz_EASS(c,j)
                fi = h2osoi_ice_EASS(c,j) / wx
                td = tfrz-t_soisno_EASS(c,j)
                dexpf = exp(-c4*td)
                ...
                if (h2osoi_liq_EASS(c,j) > 0.01_r8*dz_EASS(c,j)) ddz1=ddz1*c5
                ...
                if (imelt_EASS(c,j) == 1) then
                   ddz3 = - 1._r8/dtime * max(0._r8,(frac_iceold_EASS(c,j) - fi)/frac_iceold_EASS(c,j))
                else
                   ddz3 = 0._r8
                end if
                ...
                dz_EASS(c,j) = max(dz_EASS(c,j) * (1._r8+pdzdtc*dtime),h2osoi_ice_EASS(c,j)/denice &
                                         + h2osoi_liq_EASS(c,j)/denh2o)
             end if
  ...
  end subroutine SnowCompaction_EASS

!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: CombineSnowLayers_EASS
!
! !INTERFACE:
  subroutine CombineSnowLayers_EASS(lbc, ubc, num_snowc_EASS, filter_snowc_EASS)
!
! !DESCRIPTION:
...
! !ARGUMENTS:
    implicit none
    integer, intent(in)    :: lbc, ubc                    ! column bounds
    integer, intent(inout) :: num_snowc_EASS              ! number of column snow points in column filter
    integer, intent(inout) :: filter_snowc_EASS(ubc-lbc+1)     ! column filter for snow points
!
! !CALLED FROM:
! subroutine Hydrology2 in module Hydrology2Mod
!
...
!#    snw_rds    => clm3%g%l%c%cps%snw_rds
    ...
    do fc = 1, num_snowc_EASS
       ...
       l = clandunit(c)
       do j = msn_old(c)+1,0
          if (h2osoi_ice_EASS(c,j) <= .1_r8) then
             if (ltype(l) == istsoil .or. ltype(l)==isturb .or. ltype(l) == istcrop) then
                h2osoi_liq_EASS(c,j+1) = h2osoi_liq_EASS(c,j+1) + h2osoi_liq_EASS(c,j)
                h2osoi_ice_EASS(c,j+1) = h2osoi_ice_EASS(c,j+1) + h2osoi_ice_EASS(c,j)

                if (j /= 0) dz_EASS(c,j+1) = dz_EASS(c,j+1) + dz_EASS(c,j)
...
!#                end if
                ...
                h2osoi_liq_EASS(c,j+1) = h2osoi_liq_EASS(c,j+1) + h2osoi_liq_EASS(c,j)
                h2osoi_ice_EASS(c,j+1) = h2osoi_ice_EASS(c,j+1) + h2osoi_ice_EASS(c,j)
                dz_EASS(c,j+1) = dz_EASS(c,j+1) + dz_EASS(c,j)
...
!#                mss_dst4(c,j+1)  = mss_dst4(c,j+1) + mss_dst4(c,j)
             ...
             if (j > snl_EASS(c)+1 .and. snl_EASS(c) < -1) then
                do i = j, snl_EASS(c)+2, -1
                   t_soisno_EASS(c,i)   = t_soisno_EASS(c,i-1)
                   h2osoi_liq_EASS(c,i) = h2osoi_liq_EASS(c,i-1)
                   h2osoi_ice_EASS(c,i) = h2osoi_ice_EASS(c,i-1)
...
!#                   snw_rds(c,i)     = snw_rds(c,i-1)
                   ...
                   dz_EASS(c,i)         = dz_EASS(c,i-1)
                end do
             end if
             snl_EASS(c) = snl_EASS(c) + 1
          end if
       end do
    end do
    ...
    do j = -nlevsno+1,0
       do fc = 1, num_snowc_EASS
          ...
          if (j >= snl_EASS(c)+1) then
             ...
             snowdp_EASS(c) = snowdp_EASS(c) + dz_EASS(c,j)
             zwice(c)  = zwice(c) + h2osoi_ice_EASS(c,j)
             zwliq(c)  = zwliq(c) + h2osoi_liq_EASS(c,j)
          end if
       end do
    end do
    ...
    do fc = 1, num_snowc_EASS
       c = filter_snowc_EASS(fc)
       l = clandunit(c)
       if (snowdp_EASS(c) < 0.01_r8 .and. snowdp_EASS(c) > 0._r8) then
          snl_EASS(c) = 0
          h2osno_EASS(c) = zwice(c)
...
!#          mss_dst4(c,:)  = 0._r8
          ...
          if (h2osno_EASS(c) <= 0._r8) snowdp_EASS(c) = 0._r8
          if (ltype(l) == istsoil .or. ltype(l) == isturb .or. ltype(l) == istcrop) then
             h2osoi_liq_EASS(c,1) = h2osoi_liq_EASS(c,1) + zwliq(c)
          end if
       end if
    end do
    ...
    do fc = 1, num_snowc_EASS
       ...
       if (snl_EASS(c) < -1) then

          msn_old(c) = snl_EASS(c)
          mssi(c) = 1

          do i = msn_old(c)+1,0
             if (dz_EASS(c,i) < dzmin(mssi(c))) then

                if (i == snl_EASS(c)+1) then
                   ...
                   neibor = i + 1
                   if ((dz_EASS(c,i-1)+dz_EASS(c,i)) < (dz_EASS(c,i+1)+dz_EASS(c,i))) neibor = i-1
                end if
...
!#                               (h2osoi_liq(c,j)+h2osoi_ice(c,j)+h2osoi_liq(c,l)+h2osoi_ice(c,l))
                ...
                call Combo (dz_EASS(c,j), h2osoi_liq_EASS(c,j), h2osoi_ice_EASS(c,j), &
                   t_soisno_EASS(c,j), dz_EASS(c,l), h2osoi_liq_EASS(c,l), h2osoi_ice_EASS(c,l), t_soisno_EASS(c,l) )

                ! Now shift all elements above this down one.
                if (j-1 > snl_EASS(c)+1) then
                   do k = j-1, snl_EASS(c)+2, -1
                      t_soisno_EASS(c,k) = t_soisno_EASS(c,k-1)
                      h2osoi_ice_EASS(c,k) = h2osoi_ice_EASS(c,k-1)
                      h2osoi_liq_EASS(c,k) = h2osoi_liq_EASS(c,k-1)
...
!#                      snw_rds(c,k)   = snw_rds(c,k-1)
                      ...
                      dz_EASS(c,k) = dz_EASS(c,k-1)
                   end do
                end if

                ! Decrease the number of snow layers
                snl_EASS(c) = snl_EASS(c) + 1
                if (snl_EASS(c) >= -1) EXIT
    ...
    do j = 0, -nlevsno+1, -1
       do fc = 1, num_snowc_EASS
          c = filter_snowc_EASS(fc)
          if (j >= snl_EASS(c) + 1) then
             z_EASS(c,j) = zi_EASS(c,j) - 0.5_r8*dz_EASS(c,j)
             zi_EASS(c,j-1) = zi_EASS(c,j) - dz_EASS(c,j)
          end if
       end do
    end do

  end subroutine CombineSnowLayers_EASS

!-----------------------------------------------------------------------
!BOP
!
! !IROUTINE: DivideSnowLayers_EASS
!
! !INTERFACE:
  subroutine DivideSnowLayers_EASS(lbc, ubc, num_snowc_EASS, filter_snowc_EASS)
!
! !DESCRIPTION:
...
!#    snw_rds    => clm3%g%l%c%cps%snw_rds
    ...
    do j = 1,nlevsno
       do fc = 1, num_snowc_EASS
          c = filter_snowc_EASS(fc)
          if (j <= abs(snl_EASS(c))) then
             dzsno(c,j) = dz_EASS(c,j+snl_EASS(c))
             swice(c,j) = h2osoi_ice_EASS(c,j+snl_EASS(c))
             swliq(c,j) = h2osoi_liq_EASS(c,j+snl_EASS(c))
             tsno(c,j)  = t_soisno_EASS(c,j+snl_EASS(c))
...
!#             rds(c,5) = rds(c,4) ! (combo)
    ...
    do j = -nlevsno+1,0
       do fc = 1, num_snowc_EASS
          c = filter_snowc_EASS(fc)
          if (j >= snl_EASS(c)+1) then
             dz_EASS(c,j)         = dzsno(c,j-snl_EASS(c))
             h2osoi_ice_EASS(c,j) = swice(c,j-snl_EASS(c))
             h2osoi_liq_EASS(c,j) = swliq(c,j-snl_EASS(c))
             t_soisno_EASS(c,j)   = tsno(c,j-snl_EASS(c))
...
!#             snw_rds(c,j)     = rds(c,j-snl(c))
  ...
  end subroutine DivideSnowLayers_EASS
...
!
! !CALLED FROM:
! subroutine CombineSnowLayers_EASS in this module
! subroutine DivideSnowLayers_EASS in this module
!
! !REVISION HISTORY:
! 15 September 1999: Yongjiu Dai; Initial code
! 15 December 1999:  Paul Houser and Jon Radakovich; F90 Revision
...
!BOP
!
! !IROUTINE: BuildSnowFilter_EASS
!
! !INTERFACE:
  subroutine BuildSnowFilter_EASS(lbc, ubc, num_nolakec, filter_nolakec, &
                             num_snowc_EASS, filter_snowc_EASS, & 
                             num_nosnowc_EASS, filter_nosnowc_EASS)
!
! !DESCRIPTION:
...
! !ARGUMENTS:
    ...
    integer, intent(out) :: num_snowc_EASS                   ! number of column snow points in column filter
    integer, intent(out) :: filter_snowc_EASS(ubc-lbc+1)     ! column filter for snow points
    integer, intent(out) :: num_nosnowc_EASS                 ! number of column non-snow points in column filter
    integer, intent(out) :: filter_nosnowc_EASS(ubc-lbc+1)   ! column filter for non-snow points
!
! !CALLED FROM:
...
!-----------------------------------------------------------------------
    ...
    do fc = 1, num_nolakec
       ...
       if (snl_EASS(c) < 0) then
          num_snowc_EASS = num_snowc_EASS + 1
          filter_snowc_EASS(num_snowc_EASS) = c
       else
          num_nosnowc_EASS = num_nosnowc_EASS + 1
          filter_nosnowc_EASS(num_nosnowc_EASS) = c
       end if
    end do

  end subroutine BuildSnowFilter_EASS

end module SnowHydrology_EASSMod
