...
! If radiative forcing is being calculated, first estimate clean-snow albedo
    ...
    do ib = 1, numrad
       do fp = 1,num_vegsol
          p = filter_vegsol(fp)
          rho(p,ib) = max( rhol(ivt(p),ib)*wl(p) + rhos(ivt(p),ib)*ws(p), mpe )    !# for Sen.!# for Sen.
          tau(p,ib) = max( taul(ivt(p),ib)*wl(p) + taus(ivt(p),ib)*ws(p), mpe )    !# for Sen.
       end do
    end do
    ...
    do ib = 1, nband
       do fc = 1,num_nourbanc
          ...
          if (coszen(c) > 0._r8) then
            ...
            !# albsod(c,ib)=1.2_r8*albsod(c,ib)       !# for Sen.
            !# albsoi(c,ib)=1.2_r8*albsoi(c,ib)       !# for Sen.
