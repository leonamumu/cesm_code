...
!-----------------------------------------------------------------------
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='SNLSNO', data=cptr%cps%snl, &
            ...
            ncid=ncid, flag=flag, readvar=readvar)
            !---------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cps%snl_EASS=cptr%cps%snl
            !---------------------
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='SNOWDP', data=cptr%cps%snowdp, &
            ...
            !---------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cps%snowdp_EASS=cptr%cps%snowdp
            !---------------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='ZWT', data=cptr%cws%zwt, &
            ...
            !---------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cws%zwt_EASS=cptr%cws%zwt
            !---------------------
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='DZSNO', data=temp2d, &
            ...
            !---------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cps%dz_EASS(begc:endc,-nlevsno+1:0)=cptr%cps%dz(begc:endc,-nlevsno+1:0)
            !---------------------
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='ZSNO', data=temp2d, &
            ...
            !---------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cps%z_EASS(begc:endc,-nlevsno+1:0)=cptr%cps%z(begc:endc,-nlevsno+1:0)
            !---------------------
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='ZISNO', data=temp2d, &
            ...
            !---------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cps%zi_EASS(begc:endc,-nlevsno+1:0)=cptr%cps%zi(begc:endc,-nlevsno+1:0)
            !---------------------
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='gdir', data=pptr%pps%gdir, &
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='omega', data=pptr%pps%omega, &
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='albd', data=pptr%pps%albd, &
            dim1name=namep, switchdim=.true., ncid=ncid, flag=flag, readvar=readvar) 
            write (iulog,*) 'pptr%pps%albd=',pptr%pps%albd
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) then
             call endrun()
          else
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='albi', data=pptr%pps%albi, &
            dim1name=namep, switchdim=.true., ncid=ncid, flag=flag, readvar=readvar) 
 !#           write (iulog,*) 'pptr%pps%albi=',pptr%pps%albi
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) then
             call endrun()
          else
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='albgrd', data=cptr%cps%albgrd, &
            ...
            cptr%cps%albsod=cptr%cps%albgrd
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='albgri', data=cptr%cps%albgri, &
            dim1name=namec, switchdim=.true., ncid=ncid, flag=flag, readvar=readvar) 
 !#           write (iulog,*) 'cptr%cps%albgri=',cptr%cps%albgri
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    end if
   ...
   ! column water state variable - h2osno
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='H2OSNO', data=cptr%cws%h2osno, &
            ...
            !-------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cws%h2osno_EASS=cptr%cws%h2osno
            cptr%cws%mass_snow_grnd_EASS=cptr%cws%h2osno
            !-------------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='H2OSOI_LIQ', data=cptr%cws%h2osoi_liq, &
            ...
            !-------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cws%h2osoi_liq_EASS=cptr%cws%h2osoi_liq
            !-------------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='H2OSOI_ICE', data=cptr%cws%h2osoi_ice, &
            ...
            !-------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cws%h2osoi_ice_EASS=cptr%cws%h2osoi_ice
            !-------------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
   ...
   ! column energy state variable - t_grnd
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='T_GRND', data=cptr%ces%t_grnd, &
            ...
            !-------------------
            ! added by Jing Chen Oct 31 2012
            cptr%ces%t_grnd_EASS=cptr%ces%t_grnd
            !-------------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
   ...
   ! column energy state variable - t_grnd_u
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='T_GRND_U', data=cptr%ces%t_grnd_u, &
            ...
            !---------------------
            ! added by Jing Chen Oct 31 2012
            cptr%ces%t_grnd_u_EASS=cptr%ces%t_grnd_u
            !---------------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
   ...
   ! column energy state variable - t_grnd_r
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='T_GRND_R', data=cptr%ces%t_grnd_r, &
            ...
            !-----------------
            ! added by Jing Chen Oct 31 2012
            cptr%ces%t_grnd_r_EASS=cptr%ces%t_grnd_r
            !-----------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
   ...
   ! pft energy state variable - t_ref2m_max_inst_r
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='T_SOISNO', data=cptr%ces%t_soisno, &
            ...
            !-----------------------
            ! added by Jing Chen
            cptr%ces%t_soisno_EASS=cptr%ces%t_soisno
            !---------------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='elai', data=pptr%pps%elai, &
            ...
            do p = begp, endp
               ...
               if ( pptr%pps%elai(p) == 0.0_r8 ) then
                  pptr%pps%elai_EASS(p) = 0.1_r8
               else
                  pptr%pps%elai_EASS(p) = pptr%pps%elai(p)
               end if
               pptr%pps%elai_over_EASS(p) = pptr%pps%elai_EASS(p)
               pptr%pps%elai_under_EASS(p)=1.18_r8*exp(-0.99_r8* pptr%pps%elai_over_EASS(p))
               if ( pptr%pps%elai_under_EASS(p) > pptr%pps%elai_over_EASS(p) )then
                   pptr%pps%elai_under_EASS(p)=0.01_r8
               end if
            end do
            !---------------------
       if (flag=='read' .and. .not. readvar) then
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='T_VEG', data=pptr%pes%t_veg, &
            ...
            !----------------
            ! added by Jing Chen Oct 31 2012 
            pptr%pes%Tleaf_over_sunlit_EASS = pptr%pes%t_veg
            pptr%pes%Tleaf_under_sunlit_EASS= pptr%pes%t_veg
            pptr%pes%Tleaf_over_shaded_EASS = pptr%pes%t_veg
            pptr%pes%Tleaf_under_shaded_EASS= pptr%pes%t_veg
            !----------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='H2OCAN', data=pptr%pws%h2ocan, &
            ...
            !--------------------
            ! added by Jing Chen Oct 31 2012
            pptr%pws%h2ocan_EASS=pptr%pws%h2ocan
            !-------------------
       if (flag=='read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
   ...
   ! column irrigation variable - irrig_rate
    ...
    if (flag == 'read' ) then
       do c = begc,endc
          ...
          do j = 1,nlevs
             ...
             !------------------
             ! added by Jing Chen Oct 31 2012
             cptr%cws%h2osoi_vol_EASS(c,j)=cptr%cws%h2osoi_vol(c,j)
             !----------------
          end do
       end do
          ...
          do c = begc,endc
             ...
             do j = 1,nlevs
                ...
                if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
                   ...
                   !------------------
                   ! added by Jing Chen Oct 31 2012
                   cptr%cws%h2osoi_liq_EASS(c,j)=cptr%cws%h2osoi_liq(c,j)
                   cptr%cws%h2osoi_ice_EASS(c,j)=cptr%cws%h2osoi_ice(c,j)
                   cptr%cws%h2osoi_vol_EASS(c,j)=cptr%cws%h2osoi_vol(c,j)
                   !----------------
                end if
             end do
    ...
    if (flag == 'define') then
       ...
       call ncd_io(varname='qflx_snofrz_lyr', data=cptr%cwf%qflx_snofrz_lyr, &
            ...
            !------------------
            ! added by Jing Chen Oct 31 2012
            cptr%cwf%qflx_snofrz_lyr_EASS=cptr%cwf%qflx_snofrz_lyr
            !------------------
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) then
             call endrun()
             ...
             do c=begc,endc
                ...
                !--------------------
                ! added by Jing Chen Oct 31 2012
                cptr%cwf%qflx_snofrz_lyr_EASS(c,-nlevsno+1:0) =cptr%cwf%qflx_snofrz_lyr(c,-nlevsno+1:0)
                !--------------------
             enddo
          endif
       end if
