...
!--------------------------
!  added by Jing Chen, Sep 23 2012
!#   public :: Stomata     !Leaf stomatal resistance and leaf photosynthesis
!--------------------------
...
!   algorithms. 
! 9/23/12, Jing Chen: divided old 'one-big-leaf' model into 'two-big-leaf' model
!   in latent and sensible heat flux calculation based on CLM4 algorithms.
! 
!EOP
!------------------------------------------------------------------------------
...
! !USES:
    ...
    use clm_varcon         , only : sb, cpair, hvap, hsub, vkc, grav, denice, &
    ...
    use AirResist_EASSMod      , only : AirResist_EASS
    use EnergyBalance_EASSMod  , only : EnergyBalance_EASS
    use LeafTemper_EASSMod     , only : LeafTemper_EASS
    use PenmanEqu_EASSMod      , only : LeafFlux_EASS, CanopyFlux_EASS, GrndFlux_EASS, LHvaporFunc_EASS 
    !---------------------------------
!
! !ARGUMENTS:
    implicit none
...
!
   ...
   real(r8), pointer :: frac_snow_over_EASS(:)   ! fraction of snow on the overstory(0~1)
   ...
   real(r8), pointer :: Tleaf_new_over_shaded_EASS(:)  ! updated leaf temperature of shaded leaves,overstroy[K]
   real(r8), pointer :: Tleaf_new_over_sunlit_EASS(:)  ! updated leaf temperature of sunlit leaves,overstroy[K]
   real(r8), pointer :: Tleaf_new_under_shaded_EASS(:) ! updated leaf temperature of shaded leaves,understroy[K]
   real(r8), pointer :: Tleaf_new_under_sunlit_EASS(:) ! updated leaf temperature of sunlit leaves,understroy[K]
   ...
   real(r8), pointer :: Tleaf_sunlit_EASS(:)           ! sunlit leaf temperature [K]
   real(r8), pointer :: Tleaf_shaded_EASS(:)           ! shaded leaf temperature [K]
   ...
   real(r8), pointer :: lt_over_shaded_EASS(:)         ! (elai+esai) of shaded leaf in overstory, EASS[-]
   real(r8), pointer :: lt_over_sunlit_EASS(:)         ! (elai+esai) of sunlit leaf in overstory, EASS[-]
   real(r8), pointer :: lt_under_shaded_EASS(:)        ! (elai+esai) of shaded leaf in understory, EASS[-]
   real(r8), pointer :: lt_under_sunlit_EASS(:)        ! (elai+esai) of sunlit leaf in understory, EASS[-]
   real(r8), pointer :: forc_rh(:)                  ! relatively humidity [%]
   real(r8), pointer :: psn_over_sunlit_EASS(:)     ! foliage photosynthesis [umol co2 m-2 s-1]
   real(r8), pointer :: psn_over_shaded_EASS(:)     ! foliage photosynthesis [umol co2 m-2 s-1]
   real(r8), pointer :: psn_under_sunlit_EASS(:)    ! foliage photosynthesis [umol co2 m-2 s-1]
   real(r8), pointer :: psn_under_shaded_EASS(:)    ! foliage photosynthesis [umol co2 m-2 s-1]
   real(r8), pointer :: elai_over_sunlit_EASS(:)    ! elai of sunlit leaves, overstory [-]
   real(r8), pointer :: elai_over_shaded_EASS(:)    ! elai of shaded leaves, overstory [-]
   real(r8), pointer :: elai_under_sunlit_EASS(:)   ! elai of sunlit leaves, understory [-]
   real(r8), pointer :: elai_under_shaded_EASS(:)   ! elai of shaded leaves, understory [-]
   real(r8), pointer :: forc_t_col(:)               ! atmospheric temperature [Kelvin] at the column level
   real(r8), pointer :: radnet_grnd_EASS(:)         ! total solar radiation aborbed by the ground [W m-2]
   real(r8), pointer :: ra_grnd_EASS(:)             ! air resistance of grnd [s m-1]
   real(r8), pointer :: densnow_EASS(:)             ! snow denisity on the ground [kg m-3]
   real(r8), pointer :: h2osno_EASS(:)              ! snow water [mm H2O]=kg m-2
   ...
   real(r8), pointer :: t_grnd_EASS(:)              ! ground surface temperature [K]
...
!
   ...
   !---------------------------
   ! added by Jing Chen, 28 May,2012
   ...
   real(r8), pointer :: h2ocan_EASS(:)          ! canopy water (mm H2O)
   !-----------------------------
!
...
!
   ...
   real(r8), pointer :: eflx_sh_under_EASS(:)            ! sensible heat flux, understory [w m-2]
   real(r8), pointer :: eflx_sh_sunlit_EASS(:)           ! sensible heat flux, sunlit leaves [w m-2]
   real(r8), pointer :: eflx_sh_shaded_EASS(:)           ! sensible heat flux, shaded leaves [w m-2]
   real(r8), pointer :: eflx_sh_over_sunlit_EASS(:)      ! sensible heat flux of sunlit leaves, overstory [w m-2] 
   real(r8), pointer :: eflx_sh_under_sunlit_EASS(:)     ! sensible heat flux of sunlit leaves, understory [w m-2] 
   real(r8), pointer :: eflx_sh_over_shaded_EASS(:)      ! sensible heat flux of shaded leaves, overstory [w m-2] 
   real(r8), pointer :: eflx_sh_under_shaded_EASS(:)     ! sensible heat flux of shaded leaves, understory [w m-2] 
   real(r8), pointer :: qflx_trans_over_EASS(:)          ! transpiration from overstory[mmH2O s-1]       Trans_o[kkk]
   real(r8), pointer :: qflx_trans_under_EASS(:)         ! transpiration from understory[mmH2O s-1]       Trans_u[kkk]
   real(r8), pointer :: qflx_trans_sunlit_EASS(:)        ! transpiration from sunlit leaves[mmH2O s-1] 
   real(r8), pointer :: qflx_trans_shaded_EASS(:)        ! transpiration from shaded leaves[mmH2O s-1] 
   real(r8), pointer :: qflx_trans_over_sunlit_EASS(:)   ! transpiration of sunlit leaves, overstory[w m-2]
   real(r8), pointer :: qflx_trans_under_sunlit_EASS(:)  ! transpiration of sunlit leaves, understory[w m-2]
   real(r8), pointer :: qflx_trans_over_shaded_EASS(:)   ! transpiration of shaded leaves, overstory[w m-2]
   real(r8), pointer :: qflx_trans_under_shaded_EASS(:)  ! transpiration of shaded leaves, understory[w m-2]
   real(r8), pointer :: qflx_evap_over_EASS(:)           ! water(liquid) evaporation from overstroy[mmH2O s-1]     Eil_o[kkk]
   real(r8), pointer :: qflx_evap_under_EASS(:)          ! water(liquid) evaporation from understory[mmH2O s-1]      Eil_u[kkk]
   real(r8), pointer :: qflx_evap_sunlit_EASS(:)         ! evaporation of sunlit leaves[w m-2]
   real(r8), pointer :: qflx_evap_shaded_EASS(:)         ! evaporation of shaded leaves[w m-2]
   real(r8), pointer :: qflx_evap_over_sunlit_EASS(:)    ! evaporation of sunlit leaves, overstory[w m-2]      
   real(r8), pointer :: qflx_evap_under_sunlit_EASS(:)   ! evaporation of sunlit leaves, understory[w m-2]     
   real(r8), pointer :: qflx_evap_over_shaded_EASS(:)    ! evaporation of shaded leaves, overstory[w m-2]     
   real(r8), pointer :: qflx_evap_under_shaded_EASS(:)   ! evaporation of shaded leaves, understory[w m-2]
   real(r8), pointer :: qflx_evap_snow_over_EASS(:)      ! evaporation from snow of overstory[mmH2O s-1]      EiS_o[kkk]
   real(r8), pointer :: qflx_evap_snow_under_EASS(:)     ! evaporation from snow of understory[mmH2O s-1]     EiS_u[kkk]
   real(r8), pointer :: qflx_evap_snow_sunlit_EASS(:)    ! evaporation of sunlit leaves covered by snow[w m-2]
   real(r8), pointer :: qflx_evap_snow_shaded_EASS(:)    ! evaporation of shaded leaves covered by snow [w m-2]
   real(r8), pointer :: qflx_evap_snow_over_sunlit_EASS(:)  ! evaporation of sunlit leaves covered by snow, overstory[w m-2]      
   real(r8), pointer :: qflx_evap_snow_under_sunlit_EASS(:) ! evaporation of sunlit leaves covered by snow, understory[w m-2]     
   real(r8), pointer :: qflx_evap_snow_over_shaded_EASS(:)  ! evaporation of shaded leaves covered by snow, overstory [w m-2]     
   real(r8), pointer :: qflx_evap_snow_under_shaded_EASS(:) ! evaporation of shaded leaves covered by snow, understory[w m-2]
   ...
   real(r8), pointer :: fpsn_over_sunlit_EASS(:)    ! photosynthesis of sunlit leaves, overstory [umol CO2 m-2 s-1]
   real(r8), pointer :: fpsn_over_shaded_EASS(:)    ! photosynthesis of shaded leaves, overstory [umol CO2 m-2 s-1]
   real(r8), pointer :: fpsn_under_sunlit_EASS(:)   ! photosynthesis of sunlit leaves, understory [umol CO2 m-2 s-1]
   real(r8), pointer :: fpsn_under_shaded_EASS(:)   ! photosynthesis of shaded leaves, understory [umol CO2 m-2 s-1]
   real(r8), pointer :: gpp_fpsn_EASS(:)            ! GPP flux based on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn_over_EASS(:)       ! GPP flux of overstory based on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn_under_EASS(:)      ! GPP flux of understory based on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn_sunlit_EASS(:)     ! GPP flux of sunlit leaves based on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn_shaded_EASS(:)     ! GPP flux of shaded leaves based on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn(:)                 ! GPP flux based on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn_sunlit(:)          ! GPP flux of sunlit leaves based on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: gpp_fpsn_shaded(:)          ! GPP flux of shaded leaves based on photosynthesis [gC m-2 s-1]
   real(r8), pointer :: qflx_evap_pond_EASS(:)      ! water(liquid) evaporation from pond[mmH2O s-1]
   real(r8), pointer :: qflx_evap_grnd_EASS(:)      ! water(liquid) evaporation from grnd[mmH2O s-1]
   real(r8), pointer :: qflx_evap_snow_grnd_EASS(:) ! evaporation from snow of understory[mmH2O s-1]
   real(r8), pointer :: rs_grnd_EASS(:)             ! resistant of grnd sufance [s m-1]
   real(r8), pointer :: cpairg_EASS(:)              ! specific heat of dry air of surface ground in EASS,
                                                    ! depanding on saturated water vapor specific humidity [J kg-1 k-1]
   ...
   real(r8), pointer :: qflx_evap_soi_EASS(:)       ! water(liquid) evaporation from soil[mmH2O s-1]
                                                    ! qflx_evap_grnd_EASS+qflx_evap_pond_EASS+qflx_evap_snow_grnd_EASS
   !--------------------------
!
...
!
   ...
   !----------------------------
   ! added by Jing Chen, 20 May 2012
   integer :: cyc_index                 ! the cycle index of canopy flux calculation
   real(r8):: frac_cov                  ! fraction of canopy covered by snow or rain
   real(r8):: hvap_EASS                 ! latent heat of vaporization, depanding on temperature[J kg-1]
   real(r8):: h                         ! latent heat of vaporization or sublimation or 1

! [CML] or [cml] NOTE -----------------------------
! modified by CHE Mingliang, 2014-11-07
   real(r8),pointer:: es_EASS(:)        ! saturated vapor pressure[pa], depanding on forc_t[K] and forc_pbot[pa]
! [CML] or [cml] NOTE -----------------------------

   real(r8):: qs_EASS(lbp:ubp)          ! saturated water vapor specific humidity [kg/kg]

! [CML] or [cml] NOTE -----------------------------
! modified by CHE Mingliang, 2014-11-07
   real(r8),pointer:: espar_EASS(:)     ! partial vapour pressure[pa](=es_EASS*rh/100)
! [CML] or [cml] NOTE -----------------------------

   real(r8):: cpair_EASS(lbp:ubp)       ! specific heat of dry air in EASS,
                                        ! depanding on saturated water vapor specific humidity [J kg-1 k-1]
   real(r8):: desdT_EASS, dqsdT_EASS    ! temporary
   real(r8):: eg_EASS(lbp:ubp)          ! saturated vapor pressure of surface ground[pa], depanding on forc_t[K] and forc_pbot[pa]
   real(r8):: degdT_EASS,qsatgdT_EASS   ! temporary
   real(r8):: qsatg_EASS(lbp:ubp)       ! saturated water vapor specific humidity of surface ground [kg/kg]
   real(r8):: egpar_EASS(lbp:ubp)       ! partial vapour pressure of surface ground[pa](=eg_EASS*rh/100)
   real(r8):: de_grnd_EASS(lbp:ubp)     ! differences between  saturated vapor pressure and partial vapour pressure of surface ground[kpa]
   real(r8):: slope_grnd_EASS(lbp:ubp)  ! slopes of vapor pressure curve [kPa K-1]
   real(r8):: const                     ! a constent value in ground flux calcualtion
   real(r8):: rs_EASS                   ! ??resistant [s m-1]
   real(r8):: hvapg_EASS                ! latent heat of ground vaporization, depanding on temperature[J kg-1]
   real(r8):: a1, a2, a3, a4, a5        ! temporary var. , for cgrndl_EASS
   real(r8), parameter :: psy=0.066_r8  ! psychrometer constant[kPa k-1]
   real(r8):: Tleaf_sun_EASS
   real(r8):: Tleaf_sha_EASS
...
![CML] or [cml] NOTE---------------------------------------
   es_EASS        => clm3%g%l%c%p%pes%es_EASS
   espar_EASS     => clm3%g%l%c%p%pes%espar_EASS
![CML] or [cml] NOTE---------------------------------------
...
!--------------------------------
   ...
   Tleaf_EASS                       => clm3%g%l%c%p%pes%Tleaf_EASS
   Tleaf_over_EASS                  => clm3%g%l%c%p%pes%Tleaf_over_EASS
   Tleaf_under_EASS                 => clm3%g%l%c%p%pes%Tleaf_under_EASS
   Tleaf_sunlit_EASS                => clm3%g%l%c%p%pes%Tleaf_sunlit_EASS
   Tleaf_shaded_EASS                => clm3%g%l%c%p%pes%Tleaf_shaded_EASS
   ...
   eflx_sh_under_EASS               => clm3%g%l%c%p%pef%eflx_sh_under_EASS
   eflx_sh_sunlit_EASS              => clm3%g%l%c%p%pef%eflx_sh_sunlit_EASS
   eflx_sh_shaded_EASS              => clm3%g%l%c%p%pef%eflx_sh_shaded_EASS
   eflx_sh_under_sunlit_EASS        => clm3%g%l%c%p%pef%eflx_sh_under_sunlit_EASS
   eflx_sh_over_shaded_EASS         => clm3%g%l%c%p%pef%eflx_sh_over_shaded_EASS
   eflx_sh_under_shaded_EASS        => clm3%g%l%c%p%pef%eflx_sh_under_shaded_EASS
   eflx_sh_over_sunlit_EASS         => clm3%g%l%c%p%pef%eflx_sh_over_sunlit_EASS
   qflx_trans_over_EASS             => clm3%g%l%c%p%pwf%qflx_trans_over_EASS
   qflx_trans_under_EASS            => clm3%g%l%c%p%pwf%qflx_trans_under_EASS
   qflx_trans_sunlit_EASS           => clm3%g%l%c%p%pwf%qflx_trans_sunlit_EASS
   qflx_trans_shaded_EASS           => clm3%g%l%c%p%pwf%qflx_trans_shaded_EASS
   qflx_trans_over_sunlit_EASS      => clm3%g%l%c%p%pwf%qflx_trans_over_sunlit_EASS
   qflx_trans_under_sunlit_EASS     => clm3%g%l%c%p%pwf%qflx_trans_under_sunlit_EASS
   qflx_trans_over_shaded_EASS      => clm3%g%l%c%p%pwf%qflx_trans_over_shaded_EASS
   qflx_trans_under_shaded_EASS     => clm3%g%l%c%p%pwf%qflx_trans_under_shaded_EASS
   qflx_evap_over_EASS              => clm3%g%l%c%p%pwf%qflx_evap_over_EASS
   qflx_evap_under_EASS             => clm3%g%l%c%p%pwf%qflx_evap_under_EASS
   qflx_evap_sunlit_EASS            => clm3%g%l%c%p%pwf%qflx_evap_sunlit_EASS
   qflx_evap_shaded_EASS            => clm3%g%l%c%p%pwf%qflx_evap_shaded_EASS
   qflx_evap_over_sunlit_EASS       => clm3%g%l%c%p%pwf%qflx_evap_over_sunlit_EASS
   qflx_evap_under_sunlit_EASS      => clm3%g%l%c%p%pwf%qflx_evap_under_sunlit_EASS
   qflx_evap_over_shaded_EASS       => clm3%g%l%c%p%pwf%qflx_evap_over_shaded_EASS
   qflx_evap_under_shaded_EASS      => clm3%g%l%c%p%pwf%qflx_evap_under_shaded_EASS
   qflx_evap_snow_over_EASS         => clm3%g%l%c%p%pwf%qflx_evap_snow_over_EASS
   qflx_evap_snow_under_EASS        => clm3%g%l%c%p%pwf%qflx_evap_snow_under_EASS
   qflx_evap_snow_sunlit_EASS       => clm3%g%l%c%p%pwf%qflx_evap_snow_sunlit_EASS
   qflx_evap_snow_shaded_EASS       => clm3%g%l%c%p%pwf%qflx_evap_snow_shaded_EASS
   qflx_evap_snow_over_sunlit_EASS  => clm3%g%l%c%p%pwf%qflx_evap_snow_over_sunlit_EASS
   qflx_evap_snow_under_sunlit_EASS => clm3%g%l%c%p%pwf%qflx_evap_snow_under_sunlit_EASS
   qflx_evap_snow_over_shaded_EASS  => clm3%g%l%c%p%pwf%qflx_evap_snow_over_shaded_EASS
   qflx_evap_snow_under_shaded_EASS => clm3%g%l%c%p%pwf%qflx_evap_snow_under_shaded_EASS
   ...
   psn_over_shaded_EASS     => clm3%g%l%c%p%pcf%psn_over_shaded_EASS
   psn_over_sunlit_EASS     => clm3%g%l%c%p%pcf%psn_over_sunlit_EASS
   psn_under_shaded_EASS    => clm3%g%l%c%p%pcf%psn_under_shaded_EASS
   psn_under_sunlit_EASS    => clm3%g%l%c%p%pcf%psn_under_sunlit_EASS
   ...
   fpsn_over_sunlit_EASS    => clm3%g%l%c%p%pcf%fpsn_over_sunlit_EASS
   fpsn_over_shaded_EASS    => clm3%g%l%c%p%pcf%fpsn_over_shaded_EASS
   fpsn_under_sunlit_EASS   => clm3%g%l%c%p%pcf%fpsn_under_sunlit_EASS
   fpsn_under_shaded_EASS   => clm3%g%l%c%p%pcf%fpsn_under_shaded_EASS
   gpp_fpsn_EASS            => clm3%g%l%c%p%pcf%gpp_fpsn_EASS
   gpp_fpsn_over_EASS       => clm3%g%l%c%p%pcf%gpp_fpsn_over_EASS
   gpp_fpsn_under_EASS      => clm3%g%l%c%p%pcf%gpp_fpsn_under_EASS
   gpp_fpsn_sunlit_EASS     => clm3%g%l%c%p%pcf%gpp_fpsn_sunlit_EASS
   gpp_fpsn_shaded_EASS     => clm3%g%l%c%p%pcf%gpp_fpsn_shaded_EASS
   gpp_fpsn                 => clm3%g%l%c%p%pcf%gpp_fpsn
   gpp_fpsn_sunlit          => clm3%g%l%c%p%pcf%gpp_fpsn_sunlit
   gpp_fpsn_shaded          => clm3%g%l%c%p%pcf%gpp_fpsn_shaded
   radnet_grnd_EASS         => clm3%g%l%c%p%pef%radnet_grnd_EASS
   ra_grnd_EASS             => clm3%g%l%c%p%pps%ra_grnd_EASS
   qflx_evap_grnd_EASS      => clm3%g%l%c%p%pwf%qflx_evap_grnd_EASS
   qflx_evap_pond_EASS      => clm3%g%l%c%p%pwf%qflx_evap_pond_EASS
   qflx_evap_snow_grnd_EASS => clm3%g%l%c%p%pwf%qflx_evap_snow_grnd_EASS
   rs_grnd_EASS             => clm3%g%l%c%p%pps%rs_grnd_EASS
   cpairg_EASS              => clm3%g%l%c%p%pes%cpairg_EASS
...
!------------------------------------------------------------
 ...
 ! Modify aerodynamic parameters for sparse/dense canopy (X. Zeng)
   ...
   !========================================
   ! Calcualte leaf temperature , sensible and latent heat flux
   ! 1) CLM4: One-big-leaf
   ...
   do f=1, fn
      ...
      ! initialization
      ! leaf tempertaure[K]
 !#     Tleaf_over_sunlit_EASS(p) =forc_t(g)!-0.5_r8
 !#     Tleaf_under_sunlit_EASS(p)=forc_t(g)!-0.5_r8
 !#     Tleaf_over_shaded_EASS(p) =forc_t(g)!-0.5_r8
 !#     Tleaf_under_shaded_EASS(p)=forc_t(g)!-0.5_r8


 !#     ! sensible flux of overstory, used in AirResist_EASS, 
 ...
 !#     rs_under_shaded_EASS(p)=300.0_r8
      ...
      ci_over_sunlit_EASS(p) = 0.7_r8 * forc_pco2(g) * c3psn(ivt(p)) &
                             + 0.4_r8 * forc_pco2(g) * (1._r8-c3psn(ivt(p)))
      ci_over_shaded_EASS(p) = 0.7_r8 * forc_pco2(g) * c3psn(ivt(p)) &
                             + 0.4_r8 * forc_pco2(g) * (1._r8-c3psn(ivt(p)))
      ci_under_sunlit_EASS(p)= 0.7_r8 * forc_pco2(g) * c3psn(ivt(p)) &
                             + 0.4_r8 * forc_pco2(g) * (1._r8-c3psn(ivt(p)))
      ci_under_shaded_EASS(p)= 0.7_r8 * forc_pco2(g) * c3psn(ivt(p)) &
                             + 0.4_r8 * forc_pco2(g) * (1._r8-c3psn(ivt(p)))
      ...
      ! calculate vapor partircial pressure and specific heat capacity of air based on specific humidity 
      call QSat(forc_t(g), forc_pbot(g), es_EASS(p), desdT_EASS, qs_EASS(p), dqsdT_EASS)

      ! vapor particial pressure at tbot_g[pa]
      espar_EASS(p) = es_EASS(p)*clm_a2l%forc_rh(g)/100.0_r8

      ! specific heat capacity of air based on specific humidity[J kg-1 K-1]
      cpair_EASS(p) = cpair * (1.00_r8 + 0.84_r8 * qs_EASS(p) )

      cyc_index = 0._r8
      ITERATION2 : do while (.true.)
                  cyc_index = cyc_index + 1

                  !(1) air resitance and leaf boundary resistance
                  call AirResist_EASS(p, c, g, cpair_EASS(p)) 

                  !(2) absorbed solar radiation and net radiation of canopy, 
                  !    including overstroy and understory(sunlit and shaded) 
                  call EnergyBalance_EASS(p, c, g, espar_EASS(p)) 

                  !(3) rs and leaf temperature update, by calling 'subroutine photosynth'
                  frac_cov=1.0_r8
                  call LeafTemper_EASS(p, c, g, es_EASS(p),espar_EASS(p),cpair_EASS(p), &
                                       frac_cov, dayl_factor(p), phase='over_sunlit')
                  call LeafTemper_EASS(p, c, g, es_EASS(p),espar_EASS(p),cpair_EASS(p), &
                                       frac_cov, dayl_factor(p), phase='over_shaded')
                  call LeafTemper_EASS(p, c, g, es_EASS(p),espar_EASS(p),cpair_EASS(p), &
                                       frac_cov, dayl_factor(p), phase='under_sunlit')
                  call LeafTemper_EASS(p, c, g, es_EASS(p),espar_EASS(p),cpair_EASS(p), &
                                       frac_cov, dayl_factor(p), phase='under_shaded')

                  !(4)heat flux from canopy , including overstory and understory
                  !(4.1) sensible heat flux from leaves
                  eflx_sh_over_sunlit_EASS(p) = Gheat_over_EASS(p) * (Tleaf_new_over_sunlit_EASS(p) &
                                              - forc_t(g)) * forc_rho(g) * cpair_EASS(p)  
                  eflx_sh_under_sunlit_EASS(p)= Gheat_under_EASS(p) * (Tleaf_new_under_sunlit_EASS(p) &
                                              - forc_t(g)) * forc_rho(g) * cpair_EASS(p)
                  eflx_sh_over_shaded_EASS(p) = Gheat_over_EASS(p) * (Tleaf_new_over_shaded_EASS(p)-forc_t(g)) &
                                              * forc_rho(g) * cpair_EASS(p)
                  eflx_sh_under_shaded_EASS(p)= Gheat_under_EASS(p) * (Tleaf_new_under_shaded_EASS(p)-forc_t(g))&
                                              * forc_rho(g) * cpair_EASS(p)
                  !unit: [w m-2](= m s-1 * K * kg m-3 * J kg-1 K-1=J s-1 m-2)

                  !(4.2) sensible heat flux from canopy
                  ! eflx_sh_over_EASS(p)  used to update AirResist_EASS
                  h=1.0_r8
                  call CanopyFlux_EASS(eflx_sh_over_sunlit_EASS(p),eflx_sh_over_shaded_EASS(p), h, &
                                       lt_over_sunlit_EASS(p),lt_over_shaded_EASS(p),eflx_sh_over_EASS(p) )
                  call CanopyFlux_EASS(eflx_sh_under_sunlit_EASS(p),eflx_sh_under_shaded_EASS(p), h, &
                                       lt_under_sunlit_EASS(p),lt_under_shaded_EASS(p),eflx_sh_under_EASS(p) )

                  eflx_sh_over_EASS(p)=max(-200.0_r8,eflx_sh_over_EASS(p))
                  eflx_sh_under_EASS(p)=max(-200.0_r8,eflx_sh_under_EASS(p))
 
                  !(5) determine whether keep on 'do while' or not
                  if ((abs(Tleaf_new_over_sunlit_EASS(p)-Tleaf_over_sunlit_EASS(p)) .lt. 0.02_r8) .and. & 
                      (abs(Tleaf_new_over_shaded_EASS(p)-Tleaf_over_shaded_EASS(p)) .lt. 0.02_r8) .and. &
                      (abs(Tleaf_new_under_sunlit_EASS(p)-Tleaf_under_sunlit_EASS(p)) .lt. 0.02_r8) .and. &
                      (abs(Tleaf_new_under_shaded_EASS(p)-Tleaf_under_shaded_EASS(p)) .lt. 0.02_r8)) then
                      exit
                  else 
                      if (cyc_index .gt. 22) then 
                         Tleaf_new_over_sunlit_EASS(p)=forc_t(g)
                         Tleaf_new_over_shaded_EASS(p)=forc_t(g)
                         Tleaf_new_under_sunlit_EASS(p)=forc_t(g)
                         Tleaf_new_under_shaded_EASS(p)=forc_t(g)
                         exit
                      else
                         Tleaf_over_sunlit_EASS(p)=Tleaf_new_over_sunlit_EASS(p)
                         Tleaf_over_shaded_EASS(p)=Tleaf_new_over_shaded_EASS(p)
                         Tleaf_under_sunlit_EASS(p)=Tleaf_new_under_sunlit_EASS(p)
                         Tleaf_under_shaded_EASS(p)=Tleaf_new_under_shaded_EASS(p)
                      end if
                   end if

      end do ITERATION2  !while

      ! sensible heat flux of sunlit and shaded leaves
      h=1.0_r8
      call CanopyFlux_EASS(eflx_sh_over_sunlit_EASS(p),eflx_sh_under_sunlit_EASS(p), h, &
                           lt_over_sunlit_EASS(p),lt_under_sunlit_EASS(p),eflx_sh_sunlit_EASS(p))
      call CanopyFlux_EASS(eflx_sh_over_shaded_EASS(p),eflx_sh_under_shaded_EASS(p), h, &
                           lt_over_shaded_EASS(p),lt_under_shaded_EASS(p),eflx_sh_shaded_EASS(p))


      ! temperature of canopy
      h=lt_over_sunlit_EASS(p)+lt_over_shaded_EASS(p)
      call CanopyFlux_EASS(Tleaf_new_over_sunlit_EASS(p),Tleaf_new_over_shaded_EASS(p), h, &
                           lt_over_sunlit_EASS(p),lt_over_shaded_EASS(p),Tleaf_over_EASS(p))
      h=lt_under_sunlit_EASS(p)+lt_under_shaded_EASS(p)
      call CanopyFlux_EASS(Tleaf_new_under_sunlit_EASS(p),Tleaf_new_under_shaded_EASS(p), h, &
                          lt_under_sunlit_EASS(p),lt_under_shaded_EASS(p),Tleaf_under_EASS(p))

      h=lt_over_sunlit_EASS(p)+lt_under_sunlit_EASS(p)
      call CanopyFlux_EASS(Tleaf_new_over_sunlit_EASS(p),Tleaf_new_under_sunlit_EASS(p), h,&
                           lt_over_sunlit_EASS(p),lt_under_sunlit_EASS(p),Tleaf_sunlit_EASS(p))

      h=lt_over_shaded_EASS(p)+lt_under_shaded_EASS(p)
      call CanopyFlux_EASS(Tleaf_new_over_shaded_EASS(p),Tleaf_new_under_shaded_EASS(p),h, &
                          lt_over_shaded_EASS(p),lt_under_shaded_EASS(p),Tleaf_shaded_EASS(p))

      h=lt_over_sunlit_EASS(p)+lt_under_sunlit_EASS(p)+lt_over_shaded_EASS(p)+lt_under_shaded_EASS(p)
      call CanopyFlux_EASS(Tleaf_new_over_sunlit_EASS(p),Tleaf_new_under_sunlit_EASS(p),h,&
                           lt_over_sunlit_EASS(p),lt_under_sunlit_EASS(p),Tleaf_sun_EASS)
      call CanopyFlux_EASS(Tleaf_new_over_shaded_EASS(p),Tleaf_new_under_shaded_EASS(p),h, &
                           lt_over_shaded_EASS(p),lt_under_shaded_EASS(p),Tleaf_sha_EASS)
      Tleaf_EASS(p)=Tleaf_sun_EASS+Tleaf_sha_EASS

      !(6) water vapor from canopy
      !(6.1) transpiration from leaves and canopy,  including overstory and understroy
      !(6.1.1) transpiration from sunlit and shaded leaves
      frac_cov=1.0_r8
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), Tleaf_new_over_sunlit_EASS(p), &   ! input var.
                         es_EASS(p), espar_EASS(p), Gleaf_over_sunlit_EASS(p),frac_cov, &          ! input var.
                         qflx_trans_over_sunlit_EASS(p))                                           ! output var
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), Tleaf_new_over_shaded_EASS(p), &   ! input var.
                         es_EASS(p), espar_EASS(p), Gleaf_over_shaded_EASS(p),frac_cov, &          ! input var.
                         qflx_trans_over_shaded_EASS(p))                                           ! output var.
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), Tleaf_new_under_sunlit_EASS(p),  & ! input var.
                         es_EASS(p), espar_EASS(p), Gleaf_under_sunlit_EASS(p),frac_cov, &         ! input var.
                         qflx_trans_under_sunlit_EASS(p))                                          ! output var.
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), Tleaf_new_under_shaded_EASS(p),  & ! input var.
                         es_EASS(p), espar_EASS(p), Gleaf_under_shaded_EASS(p),frac_cov, &         ! input var.
                         qflx_trans_under_shaded_EASS(p))                                          ! output var.
     
      !(6.1.2) transpiration from canopy
      hvap_EASS=LHvaporFunc_EASS(forc_t(g))
      call CanopyFlux_EASS(qflx_trans_over_sunlit_EASS(p),qflx_trans_over_shaded_EASS(p), hvap_EASS, &
                           lt_over_sunlit_EASS(p),lt_over_shaded_EASS(p), qflx_trans_over_EASS(p))
      call CanopyFlux_EASS(qflx_trans_under_sunlit_EASS(p),qflx_trans_under_shaded_EASS(p), hvap_EASS, &
                           lt_under_sunlit_EASS(p),lt_under_shaded_EASS(p),qflx_trans_under_EASS(p))  
      call CanopyFlux_EASS(qflx_trans_over_sunlit_EASS(p),qflx_trans_under_sunlit_EASS(p), hvap_EASS, &
                           lt_over_sunlit_EASS(p),lt_under_sunlit_EASS(p),qflx_trans_sunlit_EASS(p))
      call CanopyFlux_EASS(qflx_trans_over_shaded_EASS(p),qflx_trans_under_shaded_EASS(p), hvap_EASS, &
                           lt_over_shaded_EASS(p),lt_under_shaded_EASS(p),qflx_trans_shaded_EASS(p))
      

      !(6.2) evaporation from leaves and canopy,  including overstory and understroy
      !(6.2.1)evaporation from sunlit and shaded leaves
      frac_cov=frac_rain_over_EASS(p)
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), Tleaf_new_over_sunlit_EASS(p),  &   ! input var.
                         es_EASS(p), espar_EASS(p), Gevap_over_EASS(p),frac_cov, &                  ! input var.
                         qflx_evap_over_sunlit_EASS(p))                                             ![w m-2] output var.
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), Tleaf_new_over_shaded_EASS(p), &    ! input var.
                         es_EASS(p), espar_EASS(p), Gevap_over_EASS(p),frac_cov, &                  ! input var.
                         qflx_evap_over_shaded_EASS(p))                                             ![w m-2] output var.

      frac_cov=frac_rain_under_EASS(p)
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), Tleaf_new_under_sunlit_EASS(p), &   ! input var.
                         es_EASS(p),espar_EASS(p), Gevap_under_EASS(p),frac_cov, &                  ! input var.
                         qflx_evap_under_sunlit_EASS(p))                                     ![w m-2] output var.
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), Tleaf_new_under_shaded_EASS(p), &   ! input var.
                         es_EASS(p),espar_EASS(p), Gevap_under_EASS(p),frac_cov, &                  ! input var.
                         qflx_evap_under_shaded_EASS(p))                                      ![w m-2]output var.

      !(6.2.2)evaporation from canopy
      call CanopyFlux_EASS(qflx_evap_over_sunlit_EASS(p),qflx_evap_over_shaded_EASS(p), hvap_EASS, &
                           lt_over_sunlit_EASS(p),lt_over_shaded_EASS(p) ,qflx_evap_over_EASS(p))
      call CanopyFlux_EASS(qflx_evap_under_sunlit_EASS(p),qflx_evap_under_shaded_EASS(p), hvap_EASS, &
                           lt_under_sunlit_EASS(p),lt_under_shaded_EASS(p),qflx_evap_under_EASS(p))
                           ! unit=w m-2 * kg J-1= w kg m-2 J-1 =J s-1 kg m-2 J-1=kg m-2 s-1=9.8pa s-1=1mmH2O s-1
      call CanopyFlux_EASS(qflx_evap_over_sunlit_EASS(p),qflx_evap_under_sunlit_EASS(p), hvap_EASS, &
                           lt_over_sunlit_EASS(p),lt_under_sunlit_EASS(p),qflx_evap_sunlit_EASS(p))
      call CanopyFlux_EASS(qflx_evap_over_shaded_EASS(p),qflx_evap_under_shaded_EASS(p), hvap_EASS, &
                           lt_over_shaded_EASS(p),lt_under_shaded_EASS(p),qflx_evap_shaded_EASS(p))


      !(6.3)evaporation from leaves and canopy covered by snow
      !(6.3.1)evaporation from sunlit and shaded leaves covered by snow
      frac_cov=frac_snow_over_EASS(p)
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), &
                         Tleaf_new_over_sunlit_EASS(p), es_EASS(p), &      ! input var.
                         espar_EASS(p), Gevap_over_EASS(p),frac_cov, &     ! input var.
                         qflx_evap_snow_over_sunlit_EASS(p))               ! output var.
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), &
                         Tleaf_new_over_shaded_EASS(p), es_EASS(p), &      ! input var.
                         espar_EASS(p), Gevap_over_EASS(p),frac_cov, &     ! input var.
                         qflx_evap_snow_over_shaded_EASS(p))               ! output var.

      frac_cov=frac_snow_under_EASS(p)
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p),&
                         Tleaf_new_under_sunlit_EASS(p), &                 ! input var.
                         es_EASS(p), espar_EASS(p), Gevap_under_EASS(p),frac_cov, & ! input var.
                         qflx_evap_snow_under_sunlit_EASS(p))              ! output var.
      call LeafFlux_EASS(forc_t(g), forc_rho(g), cpair_EASS(p), &
                         Tleaf_new_under_shaded_EASS(p), es_EASS(p), &     ! input var.
                         espar_EASS(p), Gevap_under_EASS(p),frac_cov, &    ! input var.
                         qflx_evap_snow_under_shaded_EASS(p))              ! output var.

      !(6.3.2) water vapor from canopy
      call CanopyFlux_EASS(qflx_evap_snow_over_sunlit_EASS(p),&
                           qflx_evap_snow_over_shaded_EASS(p), hsub, &
                           lt_over_sunlit_EASS(p),lt_over_shaded_EASS(p),&
                           qflx_evap_snow_over_EASS(p))
      call CanopyFlux_EASS(qflx_evap_snow_under_sunlit_EASS(p),&
                           qflx_evap_snow_under_shaded_EASS(p), hsub, &
                           lt_under_sunlit_EASS(p),lt_under_shaded_EASS(p),&
                           qflx_evap_snow_under_EASS(p)) 
      call CanopyFlux_EASS(qflx_evap_snow_over_sunlit_EASS(p),&
                           qflx_evap_snow_under_sunlit_EASS(p), hsub, &
                           lt_over_sunlit_EASS(p),lt_under_sunlit_EASS(p),&
                           qflx_evap_snow_sunlit_EASS(p))
      call CanopyFlux_EASS(qflx_evap_snow_over_shaded_EASS(p),&
                           qflx_evap_snow_under_shaded_EASS(p), hsub, &
                           lt_over_shaded_EASS(p),lt_under_shaded_EASS(p),&
                           qflx_evap_snow_shaded_EASS(p))
   ...
   do f = 1, fn
      ...
      ! sensible heat flux and water vapor flux from soil
      ...
      !water vapor from ground, pond and ground snow pack
      !(0) difference between saturated and partical water vapor pressure at the reference height(de_EASS)
      call QSat(t_grnd_EASS(c), forc_pbot(g), eg_EASS(p), degdT_EASS, qsatg_EASS(p), qsatgdT_EASS)

      egpar_EASS(p)   = eg_EASS(p)* forc_rh(g)/100.0_r8          ! [pa]
      de_grnd_EASS(p) = (eg_EASS(p)-egpar_EASS(p))/1000.0_r8    ! unit change: [pa]->[kpa]

      ! specific heat capacity of air based on specific humidity[J kg-1 K-1]
      cpairg_EASS(p) = cpair * (1.00_r8 + 0.84_r8 * qsatg_EASS(p) )

      ! slope of vapor pressure curve[kpa K-1]
      slope_grnd_EASS(p) = 4098.0_r8 * (eg_EASS(p)/1000.0_r8)/((t_grnd_EASS(c)-tfrz+237.3_r8)**2.0_r8) 

      !(1) sensible heat flux from ground
      eflx_sh_grnd_EASS(p) = ( t_grnd_EASS(c)-forc_t_col(c)) *forc_rho(g)*cpairg_EASS(p)/ra_grnd_EASS(p)
      ...
      rs_EASS    = 100.0_r8
      frac_cov   = 1.0_r8
      hvapg_EASS = LHvaporFunc_EASS(t_grnd_EASS(c))

      if ( (ponddp_EASS(c) .gt. 0.0_r8) .and. (snowdp_EASS(c) .eq. 0.0_r8 ) ) then 
            call GrndFlux_EASS(cpairg_EASS(p), slope_grnd_EASS(p), radnet_grnd_EASS(p), const, &  ! input var
                               de_grnd_EASS(p), ra_grnd_EASS(p), rs_EASS, hvapg_EASS, frac_cov, & ! input var
                               qflx_evap_pond_EASS(p))                                          ! output var
      else 
            qflx_evap_pond_EASS(p)=0.0_r8
      end if
      qflx_evap_pond_EASS(p)=max(-0.0002_r8 / dtime, qflx_evap_pond_EASS(p))

      if (qflx_evap_pond_EASS(p) .gt. 0.0_r8 ) qflx_evap_pond_EASS(p)=min(qflx_evap_pond_EASS(p), ponddp_EASS(c) * denh2o/dtime)   
      ponddp_EASS(c)=ponddp_EASS(c)-qflx_evap_pond_EASS(p)/denh2o*dtime          ! [m](=kg m-2 s-1 kg-1 m3 * s)

      !(3) snow
      if (snowdp_EASS(c) .gt. 0.02_r8 ) then
            frac_snow_grnd_EASS(c)=1.0_r8
      else
            frac_snow_grnd_EASS(c)=mass_snow_grnd_EASS(c)/(0.0025_r8*densnow_EASS(c))
      end if
      frac_snow_grnd_EASS(c)=min(frac_snow_grnd_EASS(c), 1.0_r8)
      frac_snow_grnd_EASS(c)=max(0.0_r8, frac_snow_grnd_EASS(c))

      const   = 0.8_r8
      rs_EASS = 100.0_r8
      if ( snowdp_EASS(c) .gt. 0.0_r8 ) then
             call GrndFlux_EASS(cpairg_EASS(p), slope_grnd_EASS(p), radnet_grnd_EASS(p), const, &   ! input var
                                de_grnd_EASS(p), ra_grnd_EASS(p), rs_EASS, hsub, frac_snow_grnd_EASS(c), &   ! input var
                                qflx_evap_snow_grnd_EASS(p))          ! output var
      else 
              qflx_evap_snow_grnd_EASS(p)=0.0_r8
      end if  
              qflx_evap_snow_grnd_EASS(p)=max(-0.0002_r8 / dtime, qflx_evap_snow_grnd_EASS(p))

      if (qflx_evap_snow_grnd_EASS(p) .gt. 0.0_r8 ) qflx_evap_snow_grnd_EASS(p)=min(qflx_evap_snow_grnd_EASS(p), mass_snow_grnd_EASS(c)/ dtime)  
      mass_snow_grnd_EASS(c)= mass_snow_grnd_EASS(c)-qflx_evap_snow_grnd_EASS(p)*dtime
  !#    h2osno_EASS(c)=mass_snow_grnd_EASS(c)

      !(4) ground surface
      rs_EASS=4.0_r8*exp(6.2-4.2* h2osoi_vol_EASS(c,1)/watsat(c,1))
     
      const=1.0_r8
      frac_cov=1.0_r8

      if ( (ponddp_EASS(c) .le. 0.0_r8) .or. (snowdp_EASS(c) .le. 0.0_r8 ) ) then
             call GrndFlux_EASS(cpairg_EASS(p), slope_grnd_EASS(p), radnet_grnd_EASS(p), const, &   ! input var
                                de_grnd_EASS(p), ra_grnd_EASS(p), rs_EASS, hvapg_EASS, frac_cov, &   ! input var
                                qflx_evap_grnd_EASS(p))          ! output var
 
             qflx_evap_grnd_EASS(p)=qflx_evap_grnd_EASS(p)*(1.0_r8 - frac_snow_grnd_EASS(c))
             qflx_evap_grnd_EASS(p)=max(0.0_r8, qflx_evap_grnd_EASS(p))
      else
             qflx_evap_grnd_EASS(p)=0.0_r8
      end if

      !(5) soil evapration
      qflx_evap_soi_EASS(p) = qflx_evap_grnd_EASS(p) + qflx_evap_snow_grnd_EASS(p) + qflx_evap_pond_EASS(p)
      ...
      cgrnds_EASS(p) = cgrnds_EASS(p) + forc_rho(g) * cpairg_EASS(p) / ra_grnd_EASS(p)

      hvap_EASS = LHvaporFunc_EASS(t_grnd_EASS(c))
      a1 = 1._r8 / hvap_EASS
      a2 = 4098._r8 * 0.8_r8 * radnet_grnd_EASS(p)
      a3 = forc_rho(g) * cpairg_EASS(p) * ( 1._r8 - forc_rh(g) ) / ra_grnd_EASS(p)
      a4 = psy *( 1._r8 + 100._r8 / ra_grnd_EASS(p) )
      a5 = t_grnd_EASS(c) - 273.15_r8 + 237.3_r8
      cgrndl_EASS(p) = cgrndl_EASS(p) + a1 * ( ( a2 * qsatgdT_EASS &
               + a3 * qsatgdT_EASS * a5 ** 2 + a3 * qsatgdT_EASS * 2._r8 * a5) &
               * ( 4098._r8 * t_grnd_EASS(c) + a4 * a5 **2._r8 ) &
               - ( a2 * t_grnd_EASS(c) + a3 * t_grnd_EASS(c) * ( a5 ** 2._r8 ) &
               * ( 4098._r8 * qsatgdT_EASS + a4 * 2._r8 * a5 ) ) )&
               / ( 4098._r8 * t_grnd_EASS(c) + a4 * a5 **2._r8 ) ** 2._r8 

      cgrnd_EASS(p)  = cgrnds_EASS(p) + cgrndl_EASS(p) * hvap_EASS
      ...
      h2ocan_EASS(p) = max(0._r8,h2ocan_EASS(p)+(qflx_trans_over_EASS(p)+ &
              qflx_trans_under_EASS(p)-qflx_evap_over_EASS(p)-qflx_evap_under_EASS(p))*dtime)

      !===================================
      ! total photosynthesis and GPP
      ! CLM4 : One-big-leaf
      ...
      ! updated GPP in CLM[gC m-2 s-1]
      ! same as the algorithm in subroutine CNAllocation and CSummary
      gpp_fpsn(p) = fpsn(p) * 12.011e-6_r8
      gpp_fpsn_sunlit(p)= ( psnsun(p)*laisun(p) )* 12.011e-6_r8
      gpp_fpsn_shaded(p)= ( psnsha(p)*laisha(p) )* 12.011e-6_r8

      ! EASS : Two-big-leaf
      ! GPP in EASS[gC m-2 s-1]
      fpsn_over_sunlit_EASS(p) = psn_over_sunlit_EASS(p) * elai_over_sunlit_EASS(p)
      fpsn_over_shaded_EASS(p) = psn_over_shaded_EASS(p) * elai_over_shaded_EASS(p)
      fpsn_under_sunlit_EASS(p)= psn_under_sunlit_EASS(p) * elai_under_sunlit_EASS(p)
      fpsn_under_shaded_EASS(p)= psn_under_shaded_EASS(p) * elai_under_shaded_EASS(p)

      gpp_fpsn_EASS(p)      = ( fpsn_over_sunlit_EASS(p) + fpsn_over_shaded_EASS(p) &
                             +  fpsn_under_sunlit_EASS(p) + fpsn_under_shaded_EASS(p) ) * 12.011e-6_r8
      gpp_fpsn_over_EASS(p) = ( fpsn_over_sunlit_EASS(p) + fpsn_over_shaded_EASS(p) ) * 12.011e-6_r8
      gpp_fpsn_under_EASS(p)= ( fpsn_under_sunlit_EASS(p) + fpsn_under_shaded_EASS(p) ) * 12.011e-6_r8

      gpp_fpsn_sunlit_EASS(p)= ( fpsn_over_sunlit_EASS(p) + fpsn_under_sunlit_EASS(p) )* 12.011e-6_r8
      gpp_fpsn_shaded_EASS(p)= ( fpsn_over_shaded_EASS(p) + fpsn_under_shaded_EASS(p) )* 12.011e-6_r8
...
#endif
     !#      vcmx(p) = 0.8_r8 * vcmx(p)    !# for Sen. test
           ...
           do iter = 1, niter
              ...
              atmp = mp(ivt(p))*psn(p)*forc_pbot(g)*cea / (cs*ei(p)) + bp        !# for Sen.
              btmp = (mp(ivt(p))*psn(p)*forc_pbot(g)/cs + bp ) * rb(p) - 1._r8   ! for Sen.
              ctmp = -rb(p)
              if (btmp >= 0._r8) then
